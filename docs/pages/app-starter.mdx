...
The <LinkTo pageId="utilities-kaizenprovider-installation">KaizenProvider</LinkTo> feeds your entire app with Kaizen defaults so it's important that it goes at the very root of your application.
```tsx
...
```
Be sure to read the <LinkTo pageId="utilities-kaizenprovider-installation">KaizenProvider</LinkTo> documentation for migration tips and in case your app requires any further config, but generally this should be it!
...
Our <LinkTo pageId="illustrations-icon-icon-future-api-specification">(Future) Icons</LinkTo> (introduced in `@kaizen/components@1.67.0`) are sourced from [Google Material Symbols](https://fonts.google.com/icons).
