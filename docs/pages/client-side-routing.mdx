...
To enable client side routing, you will need to wrap your application in a [RouterProvider](https://react-spectrum.adobe.com/react-aria/routing.html#routerprovider) from `@kaizen/components/libs/react-aria-components`. This allows you to set the `navigation` method that performs the client side routing in the `LinkButton` and `Link` component. Refer to the framework specific guidance below for [Next.js](#nextjs-config-example) or [React Router](#react-router-config-example).
...
import { RouterProvider as RacRouterProvider } from '@kaizen/components/libs/react-aria-components'
...
declare module '@kaizen/components/libs/react-aria-components' {
...
```tsx
import { RouterProvider } from '@kaizen/components/libs/react-aria-components'
