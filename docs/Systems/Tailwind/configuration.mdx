...
`<p className="kz-text-sky">...` ❌ Default TW config overwritten by `Preset`

`<p className="kz-text-myCoolNewColor">` ✅ New color added

`<p className="kz-text-purple-100">` ️❗️ Value from `Preset` overwritten to "Lime"

`<p className="kz-text-purple-200">` ✅ Value from `Preset` not overwritten
...
>
  ...
  <ul>
    ...
    <li>
      Duplication of utility classes in your CSS bundle caused by consuming components with different <code>important</code> selectors
    </li>
  </ul>
</InlineNotification>
