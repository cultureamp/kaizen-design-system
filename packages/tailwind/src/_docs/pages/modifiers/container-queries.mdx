...
Use max-width container queries to apply styles when a container is **smaller** than a certain size. This is useful for styling elements differently in constrained spaces.

<Story of={Examples.MaxWidthContainerQueries} />

## Container query ranges

Combine min-width and max-width container queries to apply styles only within specific size ranges. This gives you precise control over when styles are applied.

<Story of={Examples.ContainerQueryRanges} />
