...
* Use these tokens in your CSS. They will be compiled to CSS variables (e.g. `$kz-var-color-purple-800` will become `var(--kz-var-color-purple-800)`)
...
<CodeBlock language="scss" code={`.myClass { 
    color: mix(white, $kz-var-color-purple-800, 90%);
}`} />


Because the exposed SASS variables contain values such as `var(--kz-var-color-purple-800)`, the SASS compiler won't be able to calculate anything that relies on concrete values such as HEX strings.

Usually when we've used functions like `mix` or `shade` it has been to lighten or darken a token by a certain amount. We recommend you find the nearest plain value. In the example above, you could simply use `$kz-var-color-purple-200`.

## 4. When you need to add opacity to a colour, use `rgba()` and the `-rgb-params` variables

**Use `rgba($kz-var-color-purple-800-rgb-params, 0.5)` instead of `rgba($kz-var-color-purple-800, 0.5)` to add alpha to a color.**
...
.myClass {
    background-color: rgba($kz-var-color-neutral-200-rgb-params, 0.5);
}`} />

For similar reasons to the previous section, a statement like `rgba($kz-var-color-neutral-200, 0.5)`, will not work because neither SASS or your browser can understand the expanded value: `rgba(var(--kz-var-color-neutral-200), 0.5)`.

Every color variable such as `$kz-var-color-neutral-200` will have a matching sibling variable with the `-rgb-params` suffix. And `$kz-var-color-neutral-200-rgb-params` points to a CSS variable that contains a list of numbers, `246, 246, 246`, which browsers can then use in inside the `rgba()` function. 

For example `rgba($kz-var-color-neutral-200-rgb-params, 0.5)`, compiles to `rgba(var(--kz-var-color-neutral-200-rgb-params), 0.5)`, which the browser then resolves to `rgba(246, 246, 246, 0.5)`.
