...
.myClass {
    background-color: rgba($color-neutral-200-rgb, 0.5);
}`} />

For similar reasons to the previous section, a statement like `rgba($color-neutral-200, 0.5)`, will not work because neither SASS or your browser can understand the expanded value: `rgba(var(--color-neutral-200), 0.5)`.

Every color variable such as `$color-neutral-200` will have a matching sibling variable with the `-rgb` suffix. And `$color-neutral-200-rgb` points to a CSS variable that contains a list of numbers, `246, 246, 246`, which browsers can then use in inside the `rgba()` function. 

For example `rgba($color-neutral-200-rgb, 0.5)`, compiles to `rgba(var(--color-neutral-200-rgb), 0.5)`, which the browser then resolves to `rgba(246, 246, 246, 0.5)`.
