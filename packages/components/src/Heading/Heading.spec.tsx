...
describe("<Heading />", () => {
    ...
    it("adds a .small class instead of .large if a Heading 3 is used", () => {
        const headingMock = render(<Heading variant="heading-3" tag="div">
        Example
      </Heading>);
    ...
    it("adds a .dark-reduced-opacity class if the color prop is set to that", () => {
        const headingMock = render(<Heading variant="heading-3" color="dark-reduced-opacity">
        Example
      </Heading>);
        expect(headingMock.getByText("Example").classList).toContain("dark-reduced-opacity");
    });
    it("changes rendered HTML element when passed tag", () => {
        const headingMock = render(<Heading variant="display-0" tag="div">
        Example
      </Heading>);
        expect(headingMock.getByText("Example").tagName).toBe("DIV");
    });
    it("passes through data attributes", () => {
        const { getByTestId } = render(<Heading variant="display-0" data-testid="test-id">
        Example
      </Heading>);
    ...
    it("allows consumers to provide a className", () => {
        const { getByText } = render(<Heading variant="heading-4" classNameOverride="example-classname">
        Example
      </Heading>);
