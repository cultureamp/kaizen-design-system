 ...
 */
export const Heading = ({ children, tag, variant, color = "dark", classNameOverride, ...restProps }: HeadingProps): JSX.Element => {
    const inferredTag = tag === undefined ? translateHeadingLevelToTag(variant) : tag;
    const className = classnames(styles.heading, styles[variant], classNameOverride, styles[color], VARIANTS_24PX_OR_GREATER.includes(variant) ? styles.large : styles.small);
