...
describe("<VideoPlayer />", () => {
    ...
    it("renders a video player in the document and autoplay", async () => {
        render(<VideoPlayer autoplay fallback="illustrations/heart/spot/moods-cautionary.svg" source="illustrations/heart/spot/moods-cautionary.webm"/>);
    ...
    describe("when the aspect ratio is set as a prop", () => {
        it("has aspect ratio class", () => {
            window.matchMedia = jest
                .fn()
                .mockImplementation(() => mockDoesNotPreferReducedMotion);
            const { container } = render(<VideoPlayer aspectRatio="landscape" autoplay fallback="illustrations/heart/spot/moods-cautionary.svg" source="illustrations/heart/spot/moods-cautionary.webm"/>);
    ...
    describe("when the aspect ratio is not set as a prop", () => {
        it("does not have aspect ratio class", () => {
            window.matchMedia = jest
                .fn()
                .mockImplementation(() => mockDoesNotPreferReducedMotion);
            const { container } = render(<VideoPlayer autoplay fallback="illustrations/heart/spot/moods-cautionary.svg" source="illustrations/heart/spot/moods-cautionary.webm"/>);
    ...
    describe("Pausing/Playing animations", () => {
        it("restarts the animation when pause button is clicked", () => {
            const { container, getByRole } = render(<VideoPlayer fallback="illustrations/heart/spot/moods-cautionary.svg" source="illustrations/heart/spot/moods-cautionary.webm"/>);
