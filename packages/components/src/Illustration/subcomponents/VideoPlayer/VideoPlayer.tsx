...
export const VideoPlayer = ({ autoplay = true, loop = false, fallback, source, aspectRatio, onEnded, }: VideoPlayerProps): JSX.Element => {
    ...
    return (<figure className={classnames(styles.figure, aspectRatio && styles[aspectRatio], aspectRatio && styles.aspectRatioWrapper)}>
      <video muted={true} aria-hidden={true} preload="metadata" ref={videoRef} width="100%" data-testid="kz-video-player" className={styles.wrapper} loop={loop} poster={assetUrl(`${fallback}.png`)} autoPlay={prefersReducedMotion ? false : autoplay} playsInline={true} tabIndex={-1}>
        {isWebmCompatible && (<source src={assetUrl(`${source}.webm`)} type="video/webm"/>)}
        <source src={assetUrl(`${source}.mp4`)} type="video/mp4"/>
      </video>
      <IconButton onClick={(): void => pausePlay.toggle()} icon={pausePlay.icon} label={pausePlay.label} classNameOverride={styles.pausePlayButton}/>
    </figure>);
};
