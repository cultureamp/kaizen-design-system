...
const getInitials = (fullName?: string, max2Characters: boolean = false): string => fullName == null
    ? ""
    : fullName
        .split(/\s/)
...
const renderInitials = (fullName = "", size: AvatarSizes, disableInitials = false): JSX.Element => {
    const initials = getInitials(fullName);
    const isLongName = initials.length > 2 && size !== "small";
    const renderFallback = disableInitials || initials === "";
    return renderFallback ? (<span className={styles.fallbackIcon}>{fallbackIcon(fullName)}</span>) : (<abbr className={classnames(styles.initials, isLongName && styles.longName)} title={fullName || ""}>
      {isLongName ? (
        // Only called if 3 or more initials, fits text width for long names
        <Textfit mode="single" max={getMaxFontSizePixels(size)}>
          {initials}
        </Textfit>) : (getInitials(fullName, size === "small"))}
    </abbr>);
};
/**
 ...
 */
export const Avatar = ({ fullName, size = "medium", avatarSrc, disableInitials = false, isCompany = false, isCurrentUser = true, classNameOverride, ...restProps }: AvatarProps): JSX.Element => {
    const [avatarState, setAvatarState] = useState<"none" | "error" | "loading" | "success">(avatarSrc ? "loading" : "none");
    ...
    return (<span className={classnames(styles.wrapper, styles[size], classNameOverride, isCompany && styles.company, isPersonal && styles.personal, isOtherUser && styles.otherUser, (avatarState === "loading" || avatarState === "error") && styles.loading)} {...restProps}>
      {avatarState !== "none" && (<img ref={image} className={classnames(styles.avatarImage, isCompany && styles.companyAvatarImage)} src={avatarSrc} onError={onImageFailure} onLoad={onImageSuccess} alt={fullName || ""}/>)}
