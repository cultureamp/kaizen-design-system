...
const renderCheckOrMixedIcon = (status: CheckedStatus, reversed: boolean): React.ReactNode => {
    if (status === "off")
        return;
    return (<span className={classnames(styles.icon, reversed && styles.reversed)}>
      {status === "on" ? (<CheckIcon role="presentation" inheritSize/>) : (<MinusIcon role="presentation" inheritSize/>)}
    </span>);
};
const getCheckedFromStatus = (checkedStatus: CheckedStatus): boolean => checkedStatus === "on";
export const Checkbox = ({ checkedStatus = "off", onCheck, reversed = false, value, classNameOverride, ...restProps }: CheckboxProps): JSX.Element => (<span className={styles.container}>
    <input ref={(node): void => {
        if (node) {
...
data-indeterminate={checkedStatus === "mixed"} type="checkbox" className={classnames(styles.checkbox, classNameOverride, reversed && styles.reversed)} checked={getCheckedFromStatus(checkedStatus)} onChange={onCheck} value={value || checkedStatus} readOnly={onCheck === undefined} {...restProps}/>
