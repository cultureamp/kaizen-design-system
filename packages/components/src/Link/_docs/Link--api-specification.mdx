...
The underline on the `Link` can be toggled using the `underlined` prop. If the `Link` is not underlined, the `icon` prop must be passed. The `icon` prop is optional if the link is underlined.
...
// Link to the client side routing page in Guides.
