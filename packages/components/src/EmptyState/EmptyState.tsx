...
import { AnimatedSceneProps, EmptyStatesAction, EmptyStatesInformative, EmptyStatesNegative, EmptyStatesNeutral, EmptyStatesPositive, } from "~components/Illustration";
 ...
 */
export const EmptyState = ({ children, id, illustrationType = "informative", layoutContext = "sidebarAndContent", headingProps, bodyText, straightCorners, isAnimated = true, loop = false, classNameOverride, ...props }: EmptyStateProps): JSX.Element => {
    const IllustrationComponent = ILLUSTRATIONS[illustrationType];
    return (<div className={classnames(classNameOverride, styles[illustrationType], styles.container, styles.zen, styles[layoutContext], straightCorners && styles.straightCorners)} id={id} {...props}>
      <div className={styles.illustrationSide}>
        {isAnimated ? (<IllustrationComponent isAnimated loop={loop} classNameOverride={styles.illustration}/>) : (<IllustrationComponent classNameOverride={styles.illustration}/>)}
      </div>
