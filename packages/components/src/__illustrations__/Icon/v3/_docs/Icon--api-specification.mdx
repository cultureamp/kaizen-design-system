import { Canvas, Meta, Controls } from "@storybook/blocks"
import { ResourceLinks, KAIOInstallation } from "~storybook/components"
import * as IconStories from "./Icon.stories"

<Meta title="Illustrations/Icon/Icon (v3)/API Specification" />

# Icon API Specification (v3)

Updated August 27, 2024

<ResourceLinks
  sourceCode="https://github.com/cultureamp/kaizen-design-system/tree/main/packages/components/src/__illustrations__/Icon/v3"
  designGuidelines="/?path=/docs/illustrations-icon-v3-usage-guidelines--docs"
/>

<KAIOInstallation exportNames={["Icon" ]} family="illustrations" version="3" />

### Set up

Add the following (CDN for the variable icon font) to your Storybook `preview-head.html` and to the `<head>` of your apps.

```html
<link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
<link rel="preload" as="style" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
```

## Overview

Icon extends [Material Symbols](https://fonts.google.com/icons), utilising the CDN.

<Canvas of={IconStories.Playground} />
<Controls of={IconStories.Playground} />

## API

### Name

See [Material Symbols](https://fonts.google.com/icons) for all valid options.

### Filled

<Canvas of={IconStories.Filled} />

### Sizes

_Default: `medium`_

_Options: `small`, `medium`, `large`, `inherit`_

The `size` prop provides 3 default sizes (`small`, `medium`, `large`)
which include [optical scaling](https://m3.material.io/styles/icons/applying-icons#b41cbc01-9b49-4a44-a525-d153d1ea1425).

The `inherit` option will inherit the font-size from its parent.
Note that it means you may also want to combine it with [optical size](#optical-size).

<Canvas of={IconStories.Sizes} />

### Mirror in RTL

The `shouldMirrorInRTL` prop mirrors the Icon in `dir="rtl"`.

Use this for directional icons based on your usage context.

<Canvas of={IconStories.MirrorInRTL} />

### Customisation

The Icon component is a `span` that utilises a font, so you can modify the icon by using font-related styles.

<Canvas of={IconStories.Customisation} />

#### Optical size

_Options: `20`, `24`, `40`, `48`_

>For the image to look the same at different sizes, the stroke weight (thickness) changes as the icon size scales. Optical Size offers a way to automatically adjust the stroke weight when you increase or decrease the symbol size.

To change the [optical size](https://m3.material.io/styles/icons/applying-icons#b41cbc01-9b49-4a44-a525-d153d1ea1425),
override the CSS variable `--icon-optical-size` (eg. in Tailwind - `[--icon-optical-size:48]`).

<Canvas of={IconStories.OpticalSize} />
<Controls of={IconStories.OpticalSize} />

### Accessibility

Icons without accompanying text need to convey their intent to the screen reader.
Add `role="img"` and an `aria-label` for these use cases.

<Canvas of={IconStories.MeaningfulIcon} />

For icons which are purely presentational, add `aria-hidden` to hide it from the screen reader.

<Canvas of={IconStories.PresentationalIcon} />
