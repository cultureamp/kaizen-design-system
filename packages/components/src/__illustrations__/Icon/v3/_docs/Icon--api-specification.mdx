...
The `size` prop provides 3 default sizes (`small`, `medium`, `large`)
which include [optical scaling](https://m3.material.io/styles/icons/applying-icons#b41cbc01-9b49-4a44-a525-d153d1ea1425).

<Canvas of={IconStories.Sizes} />

### Custom sizes

Should you need a custom size, you may either `inherit` from the parent,
or override the `font-size` (eg. `className="text-heading-1"`).

You may wish to pair this with adjusting the [optical size](#optical-size).

<Canvas of={IconStories.CustomSizes} />
...
To change the [font-weight](https://m3.material.io/styles/icons/applying-icons#d7f45762-67ac-473d-95b0-9214c791e242),
override the `font-weight` (eg. `className="font-[100]"`).

<Canvas of={IconStories.FontWeight} />
<Controls of={IconStories.FontWeight} />

### Mirror in RTL

The `shouldMirrorInRTL` prop mirrors the Icon in `dir="rtl"`.

Use this for directional icons based on your usage context.
