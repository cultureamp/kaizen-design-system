import { Canvas, Controls, Meta } from '@storybook/blocks'
import { ResourceLinks, KAIOInstallation, RenameNotice, DocHeading } from '~storybook/components'
import * as LikertScaleLegacyStories from './LikertScaleLegacy.stories'

<Meta of={LikertScaleLegacyStories} tags={['deprecated']} />

<DocHeading title="LikertScaleLegacy" docTags={['dev docs']} />

<ResourceLinks
  sourceCode="https://github.com/cultureamp/kaizen-design-system/tree/main/packages/components/src/LikertScaleLegacy"
  figma="https://www.figma.com/file/ZRfnoNUXbGZv4eVWLbF4Az/%EF%B8%8F%F0%9F%96%BC%EF%B8%8F-Component-Gallery?node-id=9%3A39905&t=P1w10jr2cpPuaayw-1"
  designGuidelines="https://cultureamp.atlassian.net/wiki/spaces/DesignSystem/pages/3082060201/Likert+Scale"
/>

<RenameNotice />

<KAIOInstallation exportNames="LikertScaleLegacy" />

## Overview

Likert scale radio buttons let people select one option in a Likert scale ranging from strongly disagree to strongly agree.

<Canvas of={LikertScaleLegacyStories.Playground} />
<Controls of={LikertScaleLegacyStories.Playground} />

## API

### isRequired

Sets aria-required value on radiogroup for assistive technologies. An accessible label must be provided and validation must still be handled within implementations.

<Canvas of={LikertScaleLegacyStories.IsRequired} />
