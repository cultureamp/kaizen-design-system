...
const SelectedItemIcon = (): JSX.Element => (<CheckIcon role="presentation" classNameOverride={styles.check}/>);
/**
 ...
 */
export const LikertScaleLegacy = ({ scale, selectedItem, reversed, colorSchema = "classical", "data-testid": dataTestId, onSelect, validationMessage, status, labelId, }: LikertScaleProps): JSX.Element => {
     ...
     */
    const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>, focusedItem: ScaleItem): void => {
    ...
    return (<div className={classnames(styles.container, isRated && styles.rated, reversed && [styles.reversed], hoveredItem !== null && styles.hovered)} aria-labelledby={labelId} role="radiogroup" tabIndex={-1} aria-describedby={validationMessageId} data-testid={dataTestId}>
      <div className={styles.legend} data-testid={dataTestId && `${dataTestId}-legend`}>
        <Text variant="small" color={reversed ? "white" : "dark"}>
          {legend}
        </Text>
      </div>
      <div className={classnames(styles.itemContainer, isRated && styles.rated, reversed && styles.reversed)}>
        {scale.map((item: ScaleItem) => {
            ...
            return (<div className={classnames(styles.likertItem, colorSchema == "blue"
                    ? styles.blueColorSchema
                    : styles.classicalColorSchema, styles[`likertItem${item.value}`], isSelected && styles.selected, isSuggested && styles.suggested, isUnselected && styles.unselected)} key={item.value} data-testid={dataTestId && `${dataTestId}-item-${item.value}`} onClick={(): void => handleRadioClick(item)} onMouseEnter={(): void => setHoveredItem(item)} onMouseLeave={(): void => setHoveredItem(null)} onKeyDown={(event): void => handleKeyDown(event, item)} onFocus={(): void => setHoveredItem(item)} onBlur={(): void => setHoveredItem(null)} role="radio" aria-label={item.label} aria-checked={isSelectedItem} aria-posinset={item.value} aria-setsize={5} tabIndex={tabIndex} ref={itemRef && itemRef.ref}>
              <div className={classnames(styles.likertItemFill, styles[`field${item.value}`], isSelectedItem && styles.pop)}/>
              {isSelectedItem ? <SelectedItemIcon /> : null}
            </div>);
        })}
      </div>
      {shouldDisplayValidationMessage && (<FieldMessage id={validationMessageId} message={validationMessage} status={status} reversed={reversed}/>)}
    </div>);
};
