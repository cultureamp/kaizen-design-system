...
export const Skirt = ({ children, variant = "default", titleBlockHasNavigation = true, classNameOverride, ...restProps }: SkirtProps): JSX.Element => {
    ...
    return (<Container ref={ref} classNameOverride={classnames(styles.container, classNameOverride)} {...restProps}>
      <div style={{ ...(skirtHeight && { height: `${skirtHeight}px` }) }} className={classnames(styles.underlay, styles[`${variant}Variant`])}/>
...
const deriveSkirtHeight = (rect: DOMRectReadOnly, titleBlockHasNavigation: boolean): number => {
    ...
    return Math.max(spacing, height > maxHeightWithSpacing
        ? derivedMaxHeight
