import React, { ComponentType, FocusEvent, MouseEvent, Ref, forwardRef, useImperativeHandle, useRef, } from "react";
...
export type ButtonFormAttributes = Pick<React.ButtonHTMLAttributes<HTMLButtonElement>, "form" | "formAction" | "formMethod" | "formEncType" | "formTarget" | "formNoValidate">;
...
export type BaseButtonProps = GenericProps & ButtonFormAttributes & {
    label: string;
    primary?: boolean;
    destructive?: boolean;
    secondary?: boolean;
...
export const GenericButton = forwardRef(({ iconPosition = "start", iconButton = false, primary = false, secondary = false, newTabAndIUnderstandTheAccessibilityImplications = false, disableTabFocusAndIUnderstandTheAccessibilityImplications = false, type = "button", ...otherProps }: RenderProps, ref: Ref<ButtonRef | undefined>) => {
    ...
    return (<span ref={wrapperRef} className={classnames(styles.container, props.fullWidth && styles.fullWidth)} aria-live={"workingLabel" in props ? "polite" : undefined}>
...
const renderCustomComponent = (CustomComponent: ComponentType<CustomButtonProps>, props: RenderProps, ref: Ref<HTMLSpanElement>): JSX.Element => {
    const passedInProps = {
        id: props.id,
    ...
    const [contextProps, contextRef] = useContextProps(passedInProps, ref, 
    ...
    return (<span ref={contextRef} className={classnames(styles.container, props.fullWidth && styles.fullWidth)} aria-live={"workingLabel" in props ? "polite" : undefined}>
      <CustomComponent {...contextProps} {...linkProps} aria-describedby={props["aria-describedby"] === null
            ? undefined
...
const renderButton = (props: RenderProps, ref: Ref<HTMLButtonElement>): JSX.Element => {
    ...
    const passedInProps: React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement> = {
        id: props.id,
        disabled: props.disabled,
    ...
    <button {...passedInProps} {...focusableProps} aria-describedby={props["aria-describedby"] === null
            ? undefined
            : props["aria-describedby"] || focusableProps["aria-describedby"]} 
    // Unset this because the one defined in buttonProps shows
    // focus-visible styles on click
    onPointerDown={undefined} ref={ref}>
      {renderContent(props)}
    </button>);
};
const renderLink = (props: RenderProps, ref: Ref<HTMLAnchorElement>): JSX.Element => {
    const target = props.newTabAndIUnderstandTheAccessibilityImplications
        ? "_blank"
        : "_self";
    const passedInProps: React.DetailedHTMLProps<React.AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement> = {
        id: props.id,
        href: props.href,
        target,
    ...
    // @ts-ignore
    const { focusableProps } = useFocusable(passedInProps, ref);
    return (<a {...passedInProps} {...focusableProps} aria-describedby={props["aria-describedby"] === null
            ? undefined
            : props["aria-describedby"] || focusableProps["aria-describedby"]} 
...
const renderLoadingSpinner = (): JSX.Element => (<div className={styles.loadingSpinner}>
    <LoadingSpinner accessibilityLabel="" size="sm"/>
  </div>);
const renderWorkingContent = (props: Extract<RenderProps, {
    working: true;
}>): JSX.Element => {
    ...
    return (<>
      ...
      {props.additionalContent && (<span className={styles.additionalContentWrapper}>
...
const renderBadge = (props: RenderProps): JSX.Element | null => {
    ...
    if (animateChange) {
        return (<BadgeAnimated variant={variant} reversed={reversed}>
        {text}
      </BadgeAnimated>);
    }
    return (<Badge variant={variant} reversed={reversed}>
      {text}
    </Badge>);
};
const renderContent = (props: RenderProps): JSX.Element => (<span className={styles.content}>
    {props.working ? renderWorkingContent(props) : renderDefaultContent(props)}
  </span>);
const renderIcon = (icon: JSX.Element): JSX.Element => (<span className={styles.iconWrapper}>{icon}</span>);
