...
describe("<GenericButton />", () => {
    it("renders the custom component element when passed the component prop", () => {
        render(<GenericButton label="button label" component={(props): React.ReactElement => (<div>
            <div>I&apos;m custom</div>
    ...
    it("does not render an anchor element when passed the href prop but has a truthy disabled prop or a truthy working prop", () => {
        const { rerender } = render(<GenericButton label="My link" href="/to-infinity" disabled={true}/>);
        ...
        rerender(<GenericButton label="My link" href="/to-infinity" working={true} workingLabel="My link working"/>);
    ...
    it("passes custom props to a custom component", () => {
        render(<GenericButton label="button label" data-testid="custom-prop" component={(props): React.ReactElement => <div {...props}/>}/>);
...
describe("<GenericButton /> with native HTML `form` attributes", () => {
    ...
    it("will pass form props into the button", () => {
        render(<GenericButton label="submit button" data-testid="custom-prop" {...buttonFormAttributes}/>);
        expect(screen.getByRole("button", { name: "submit button" })).toHaveAttribute("form", buttonFormAttributes.form);
    });
});
describe("<GenericButton /> `working` accessible states", () => {
    it("renders a button without aria-live by default", () => {
        const { getByTestId } = render(<GenericButton data-testid="id--generic-test" id="id--button" label="button label"/>);
    ...
    it("renders a button with aria-live if working label if provided", () => {
        const { getByTestId } = render(<GenericButton data-testid="id--generic-test" id="id--button" label="button label" workingLabel="Loading"/>);
        const button = getByTestId("id--generic-test");
        const buttonContainer = button.parentElement;
    ...
    it("renders a link button with aria-live if working label if provided", () => {
        const { getByTestId } = render(<GenericButton data-testid="id--generic-test" id="id--button" label="button label" workingLabel="Loading" href="/"/>);
        const button = getByTestId("id--generic-test");
        const buttonContainer = button.parentElement;
        expect(buttonContainer).toHaveAttribute("aria-live", "polite");
    });
    it("renders a custom button with aria-live if working label if provided", () => {
        const { getByTestId } = render(<GenericButton data-testid="id--generic-test" id="id--button" label="button label" workingLabel="Loading" component={props => (<button type="button" {...props}>
            Custom button
          </button>)}/>);
        const button = getByTestId("id--generic-test");
        const buttonContainer = button.parentElement;
