...
<Meta title="Components/Button/API Specification" />
...
The `icon` property abstracts the need to handle positioning and sizing logic for icons within the `Button`. When paired with the [Icon component](/docs/components-icon-api-specification--docs), this will scale the icon to the `Button`'s `size` prop.
