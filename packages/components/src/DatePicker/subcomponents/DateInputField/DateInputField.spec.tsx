...
const DateInputFieldWrapper = (props: Partial<DateInputFieldProps>): JSX.Element => <DateInputField {...defaultProps} {...props}/>;
describe("<DateInputField />", () => {
    ...
    describe("Icon button", () => {
        ...
        it("has helpful label showing the current date when one is selected", async () => {
            renderWithIntl(<DateInputFieldWrapper value="Mar 1, 2022" onChange={(): void => undefined}/>);
            await waitFor(() => {
    ...
    describe("Validation", () => {
        it("shows validation message", async () => {
            renderWithIntl(<DateInputFieldWrapper status="error" validationMessage="There is an error"/>);
        ...
        it("does not show validation message when field is disabled", async () => {
            renderWithIntl(<DateInputFieldWrapper status="error" validationMessage="There is an error" disabled/>);
            await waitFor(() => {
    ...
    describe("Refs", () => {
        it("correctly passes through input and button refs", async () => {
            ...
            const Wrapper = (): JSX.Element => {
                ...
                const handleClick = (): void => onButtonClick(inputRef.current?.id, buttonRef.current?.getAttribute("aria-label"));
                return (<>
            <DateInputField ref={ref} id="test__date-input-field--ref" labelText="Adventure time" onButtonClick={jest.fn<void, [
                    ]>()} locale={enUS}/>
