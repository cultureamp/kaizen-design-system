...
const DatePickerWrapper = ({ selectedDay, ...restProps }: Partial<DatePickerProps>): JSX.Element => {
    const [selectedDate, setValueDate] = useState<Date | undefined>(selectedDay);
    return (<DatePicker id="test__date-picker" labelText="Input label" onDayChange={setValueDate} selectedDay={selectedDate} locale="en-US" {...restProps}/>);
};
describe("<DatePicker />", () => {
    ...
    it("re-renders the displayed input when an selectedDay is updated after initial render", async () => {
        const DelayedSelectedDate = (): JSX.Element => {
            ...
            return (<DatePicker id="test__date-picker" labelText="Input label" onDayChange={setValueDate} selectedDay={selectedDate} locale="en-US"/>);
        };
    ...
    it("should validate and close the calendar when the user presses the Enter key while focus is in the input", async () => {
        renderWithIntl(<DatePickerWrapper disabledDates={[new Date("2022-05-01")]}/>);
        const input = screen.getByLabelText("Input label", { selector: "input" });
...
describe("<DatePicker /> - Validation", () => {
    describe("Custom Validation", () => {
        it("displays custom validation message when provided (overrides inbuilt validation)", async () => {
            renderWithIntl(<DatePickerWrapper status="error" validationMessage="Custom validation message" selectedDay={new Date("potato")}/>);
            await waitFor(() => {
        ...
        it("does not show inbuilt validation message when onValidate is set", async () => {
            ...
            renderWithIntl(<DatePickerWrapper selectedDay={new Date("potato")} onValidate={onValidate}/>);
            await waitFor(() => {
        ...
        it("triggers validation when initial selected date is invalid", async () => {
            const onValidate = jest.fn();
            renderWithIntl(<DatePickerWrapper onValidate={onValidate} selectedDay={new Date("potato")}/>);
            await waitFor(() => {
        ...
        it("triggers validation when initial selected date is disabled", async () => {
            const onValidate = jest.fn();
            renderWithIntl(<DatePickerWrapper onValidate={onValidate} disabledBefore={new Date("2022-05-15")} selectedDay={new Date("2022-05-05")}/>);
            await waitFor(() => {
                expect(onValidate).toHaveBeenCalledTimes(1);
            });
        });
        it("does not trigger validation when initial selected date is empty", async () => {
            const onValidate = jest.fn();
            renderWithIntl(<DatePickerWrapper onValidate={onValidate} selectedDay={undefined}/>);
            await waitFor(() => {
        ...
        it("does not trigger validation when initial selected date is valid", async () => {
            const onValidate = jest.fn();
            renderWithIntl(<DatePickerWrapper onValidate={onValidate} selectedDay={new Date("2022-05-05")}/>);
            await waitFor(() => {
                expect(onValidate).not.toHaveBeenCalled();
            });
        });
        it("triggers validation when selected date is updated to invalid", async () => {
            const onValidate = jest.fn();
            renderWithIntl(<DatePickerWrapper onValidate={onValidate} defaultMonth={new Date("2022-03-01")}/>);
            const input = screen.getByLabelText("Input label", { selector: "input" });
            await user.click(input);
            await waitFor(() => {
                expect(screen.queryByRole("dialog")).toBeVisible();
    ...
    describe("Inbuilt Validation", () => {
        ...
        it("displays error message when selected day is disabled", async () => {
            renderWithIntl(<DatePickerWrapper disabledBefore={new Date("2022-05-15")} selectedDay={new Date("2022-05-05")}/>);
            await waitFor(() => {
        ...
        it("displays error message when input date is disabled", async () => {
            renderWithIntl(<DatePickerWrapper disabledBefore={new Date("2022-05-15")}/>);
            const input = screen.getByLabelText("Input label", { selector: "input" });
