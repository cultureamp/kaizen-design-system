...
import { CalendarSingle, CalendarSingleElement, CalendarSingleProps, DisabledDayMatchers, calculateDisabledDays, formatDateAsNumeral, formatDateAsText, isDisabledDate, isInvalidDate, isSelectingDayInCalendar, parseDateAsTextOrNumeral, setFocusInCalendar, } from "~components/Calendar";
...
} & DisabledDayMatchers & Omit<DateInputFieldProps, OmittedDateInputFieldProps>;
/**
...
export const DatePicker = ({ id: propsId, buttonRef: propsButtonRef = useRef<HTMLButtonElement>(null), locale: propsLocale = "en-AU", disabledDates, disabledDaysOfWeek, disabledRange, disabledBeforeAfter, disabledBefore, disabledAfter, weekStartsOn, defaultMonth, selectedDay, status, validationMessage, onInputClick, onInputFocus, onInputChange, onInputBlur, onButtonClick, onDayChange, onValidate, ...restDateInputFieldProps }: DatePickerProps): JSX.Element => {
    ...
    const [inbuiltValidationMessage, setInbuiltValidationMessage] = useState<ValidationResponse["validationMessage"]>();
    ...
    const handleCalendarDayChange: DayClickEventHandler = date => {
        if (!isDisabledDate(date, disabledDays)) {
            const newInputValue = lastTrigger === "calendarButton"
    ...
    return (<FocusOn scrollLock={false} returnFocus={false} onDeactivation={handleReturnFocus} onClickOutside={(): void => setIsOpen(false)} onEscapeKey={(): void => setIsOpen(false)} enabled={isOpen}>
      <div ref={containerRef}>
        <DateInputField ref={dateInputRefs} id={id} role="combobox" aria-haspopup="dialog" aria-expanded={isOpen} aria-controls={calendarId} value={inputValue} locale={locale} onButtonClick={handleButtonClick} onClick={handleInputClick} onFocus={handleInputFocus} onChange={handleInputChange} onBlur={handleInputBlur} onKeyDown={handleKeyDown} status={status !== undefined || !shouldUseInbuiltValidation
            ? status
            : inbuiltStatus} validationMessage={validationMessage !== undefined || !shouldUseInbuiltValidation
            ? validationMessage
            : inbuiltValidationMessage} {...restDateInputFieldProps}/>
      </div>

      {isOpen && (<CalendarPopover referenceElement={containerRef.current} aria-labelledby={`${id}-calendar-label ${id}-input-label`}>
          <>
            <VisuallyHidden id={`${id}-calendar-label`}>
              {calendarLabelDesc}
            </VisuallyHidden>
            <CalendarSingle id={calendarId} selected={selectedDay} defaultMonth={defaultMonth} weekStartsOn={weekStartsOn} disabled={disabledDays} locale={locale} onDayClick={handleCalendarDayChange} onMount={handleCalendarMount}/>
          </>
        </CalendarPopover>)}
    </FocusOn>);
};
