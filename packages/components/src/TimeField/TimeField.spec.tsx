...
const pressArrowKey = (direction: "ArrowUp" | "ArrowDown") => async (element: HTMLElement): Promise<void> => {
...
const TimeFieldWrapper = ({ value: propsValue = null, ...customProps }: Partial<TimeFieldProps>): JSX.Element => {
    const [value, setValue] = useState<ValueType | null>(propsValue);
    return (<TimeField locale="en-GB" id="id" label={LABEL} value={value} onChange={setValue} {...customProps}/>);
};
...
describe("onChange", () => {
    it("returns correct time from 12 hour format display", async () => {
        render(<TimeFieldWrapper value={{ hour: 16, minutes: 44 }} onChange={mockOnChange} locale="en-AU"/>);
        const hourSpinner = screen.getByRole("spinbutton", {
            name: `hour, ${LABEL}`,
        });
        expect(hourSpinner).toHaveTextContent("4");
    ...
    it("returns correct time from 24 hour format display", async () => {
        render(<TimeFieldWrapper value={{ hour: 16, minutes: 44 }} onChange={mockOnChange} locale="en-GB"/>);
        const hourSpinner = screen.getByRole("spinbutton", {
            name: `hour, ${LABEL}`,
        });
