...
export const TimeField = (props: TimeFieldProps): JSX.Element => (<I18nProvider locale={props.locale}>
    <TimeFieldComponent {...props}/>
  </I18nProvider>);
TimeField.displayName = "TimeField";
const TimeFieldComponent = ({ id: propsId, label, locale, onChange, value, status = "default", validationMessage, isDisabled, classNameOverride, ...restProps }: TimeFieldProps): JSX.Element => {
    const reactId = useId();
    ...
    return (<div className={classNameOverride}>
      ...
      <div className={styles.wrapper}>
        ...
        <div {...fieldProps} id={id} ref={inputRef} className={classnames(styles.input, state.isDisabled && styles.isDisabled, state.validationState === "invalid" && styles.error)}>
          {state.segments.map((segment, i) => (<TimeSegment key={i} segment={segment} state={state}/>))}
