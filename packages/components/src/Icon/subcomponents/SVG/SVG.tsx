 ...
 */
export const SVG = ({ inheritSize = false, role, viewBox = "0 0 20 20", classNameOverride, children, ...restProps }: SVGProps): JSX.Element => {
    const classes = classnames(styles.icon, classNameOverride, inheritSize && styles.inheritSize);
    if (role === "presentation") {
        return (<svg className={classes} viewBox={viewBox} role={role} aria-hidden={true} {...restProps}>
        {children}
      </svg>);
    }
    return (<svg role={role} className={classes} viewBox={viewBox} {...restProps}>
      {children}
    </svg>);
};
SVG.displayName = "SVG";
