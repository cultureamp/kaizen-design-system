...
import * as IconsExtendedStories from './IconsExtended.stories'
...
Custom SVG icons that extend the icon set beyond Material Symbols. These are exported directly from `@kaizen/components` alongside `Icon`.

`UserSettingsIcon` is the only custom icon currently available.

```tsx
import { UserSettingsIcon } from '@kaizen/components'
;<UserSettingsIcon size={24} color="currentColor" />
```

Props:

- **`size`** — sets both `width` and `height` in pixels. Defaults to `24`.
- **`color`** — sets the icon color. Defaults to `currentColor`.

<Canvas of={IconsExtendedStories.Playground} />
<Controls of={IconsExtendedStories.Playground} />
