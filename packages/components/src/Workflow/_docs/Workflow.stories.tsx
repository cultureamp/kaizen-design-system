...
const MockContent = (): JSX.Element => (<div className="flex flex-col flex-1 m-24 items-center">
    <div className="max-w-[1392px] w-full">
      ...
      <form id="workflow-form-id" className="bg-white rounded shadow-sm my-32 p-64">
        ...
        <TextField labelText="Cycle name" type="text" id="input-workflow" description="This is the name that will be displayed across the cycle for everyone"/>
      </form>
    </div>
  </div>);
const meta = {
    ...
    args: {
        ...
        headerActions: [
            <Button key="would-use-uui-2" label="Save and close" icon={<CloseIcon role="presentation"/>} secondary iconPosition="end"/>,
        ],
...
export const MultipleActions: Story = {
    args: {
        headerActions: [
            <Button key="would-use-uui-1" label="Preview" icon={<VisibleIcon role="presentation"/>} secondary iconPosition="start"/>,
            <Button key="would-use-uui-2" label="Save and close" icon={<CloseIcon role="presentation"/>} secondary iconPosition="end" onClick={(): void => alert("mock example of a save action")}/>,
        ],
    },
};
...
export const ComposableWorkflow: Story = {
    render: ({ steps, isComplete, workflowName, stepName, status, headerActions, previousAction, nextAction, ...restProps }) => (<Workflow.Wrapper {...restProps}>
