...
A common pattern identified is a Workflow Exit button. An implemntation of this component should be passed to the `headerActions` array to handle a user leaving a Workflow. We've provided an example below that combines a Kaizen `Button` with the `ConfirmationModal` component.
