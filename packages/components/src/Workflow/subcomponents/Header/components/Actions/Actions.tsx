...
export const Actions = ({ headerActions, ...restProps }: WorkflowActionsProps): JSX.Element => (<div className={styles.actions} {...restProps}>
    {headerActions?.map((action, index) => (<action.type key={`header-actions ${index}`} {...action.props}/>))}
  </div>);
