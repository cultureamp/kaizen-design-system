 ...
 */
export const Pagination = ({ currentPage = 1, pageCount, ariaLabelNextPage, ariaLabelPreviousPage, ariaLabelPage, onPageChange, classNameOverride, ...restProps }: PaginationProps): JSX.Element => {
    ...
    const paginationIndicator = (page: number): JSX.Element => (<PaginationLink key={page} pageNumber={page} isActive={currentPage === page} aria-label={`${ariaLabelPage} ${page}`} onClick={() => handleButtonClick(page)}/>);
    const pagination = (): JSX.Element[] => {
        ...
        const showAllPages = 1 + 2 * truncateSize + 2 * siblingPagesRange + 2 * boundaryPagesRange >=
            pageCount;
        ...
        if (showAllPages) {
            ...
            const mainPagesStart = Math.min(Math.max(currentPage - siblingPagesRange, firstPagesEnd + truncateSize + 1), lastPagesStart - truncateSize - 2 * siblingPagesRange - 1);
            ...
            items.push(showPageInsteadOfFirstEllipsis ? (paginationIndicator(firstEllipsisPageNumber)) : (<TruncateIndicator key={firstEllipsisPageNumber}/>));
            ...
            items.push(showPageInsteadOfSecondEllipsis ? (paginationIndicator(secondEllipsisPageNumber)) : (<TruncateIndicator key={secondEllipsisPageNumber}/>));
    ...
    return (<nav className={classnames(styles.container, classNameOverride)} {...restProps}>
      <DirectionalLink label={ariaLabelPreviousPage} direction="prev" disabled={previousPageDisabled} onClick={(): void => handleButtonClick("prev")}/>

      <div className={styles.pagesIndicatorWrapper}>{pagination()}</div>

      <DirectionalLink label={ariaLabelNextPage} direction="next" disabled={nextPageDisabled} onClick={(): void => handleButtonClick("next")}/>
    </nav>);
};
