...
const WithTooltip = ({ tooltipProps, children, }: WithTooltipProps): JSX.Element => !!tooltipProps ? (<Tooltip {...tooltipProps}>{children}</Tooltip>) : (<>{children}</>);
/**
 ...
 */
export const GuidanceBlock = ({ layout = "default", variant = "default", withActionButtonArrow = true, noMaxWidth = false, illustrationType = "spot", smallScreenTextAlignment = "center", actions, illustration, secondaryDismiss, ...restProps }: GuidanceBlockProps): JSX.Element => {
    ...
    return (<div className={classNames(styles.banner, variant && styles[variant], layout && styles[layout], hidden && styles.hidden, mediaQueryLayout === "centerContent" && styles.centerContent, noMaxWidth && styles.noMaxWidth, illustrationType === "scene" && styles.hasSceneIllustration, smallScreenTextAlignment === "left" && styles.smallScreenTextAlignment)} style={{
            marginTop: marginTop(),
        }} ref={containerRef} onTransitionEnd={onTransitionEnd}>
      ...
      <div className={styles.descriptionAndActions}>
        ...
        {actions?.primary && (<Media query="(max-width: 767px)">
            {(isMobile: boolean): JSX.Element => (<div className={classNames({
                    noRightMargin: isMobile || componentIsMobile,
                    rightMargin: !(isMobile || componentIsMobile) && layout === "default",
                })}>
                <div className={classNames(styles.buttonContainer, actions?.secondary && styles.secondaryAction)}>
                  <WithTooltip tooltipProps={actions.primary.tooltip}>
                    <Button icon={withActionButtonArrow ? (<ArrowForwardIcon role="presentation"/>) : undefined} iconPosition="end" {...actions.primary} fullWidth={isMobile || componentIsMobile}/>
                  </WithTooltip>

                  {actions?.secondary && (<WithTooltip tooltipProps={actions.secondary.tooltip}>
                      <div className={styles.secondaryAction}>
                        <Button secondary {...actions.secondary} onClick={secondaryDismiss
                        ? (): void => handleDismissBanner()
                        : actions?.secondary?.onClick} fullWidth={isMobile || componentIsMobile}/>
                      </div>
                    </WithTooltip>)}
                </div>
              </div>)}
