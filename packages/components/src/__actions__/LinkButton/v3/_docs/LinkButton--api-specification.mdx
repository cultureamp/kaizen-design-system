...
While client side routing is possible, the `LinkButton` is agnostic to the routing technology chosen. Refer to our [general set up guide here](#client-side-routing) to get started with client side routing.
...
// This provides the typings for the routerOptions in `LinkButton`. As the component agnostic to routing technology this must defined here
...
const Component = () => {
  ...
  return (
    <>
      ...
      <LinkButton href={`${router.pathname}/path-1`}>
        Internal link
      </LinkButton>
