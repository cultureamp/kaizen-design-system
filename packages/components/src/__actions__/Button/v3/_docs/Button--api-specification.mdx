...
The following table showcases the essential props that enable the core functionality of `Button`.
...
Labels and any button content can be passed into the `Button` as `children`. Content wrapped by the `Button` will be spaced using `flex-gap` relative to the `size` prop. This enables consistent spacing in `RTL` layouts.
...
The recommended pattern to achieve an icon only button (aka: `IconButton`) is to pass an icon in as props, then to use a [VisuallyHidden](/docs/components-visually-hidden--docs) component as `children`.
...
For an icon only `Button`, this can be paired with the `isPendingLabelHidden` prop to visibly hide the `pendingLabel` to avoid layout shifts. It is important that the `pendingLabel` is still given meaningful message as this will be announced to a screen reader.
...
### Styling hover, focus and other pseudo-classes

Pseudo-classes, such as `:hover` or `:focus-visible`, are styled via data attributes that reflect React Aria's Button internal state, ie: `isHovered`, `isFocusVisible`.

When adding styles via the `className` prop, we recommended using [React Aria's data attributes](https://react-spectrum.adobe.com/react-aria/Button.html#styling), such as `:[data-hovered]` or `:[data-focus-visible]`, to ensure styles are applied consistently.
...
The following table is a collection of additional React Aria and native HTML props that are exposed from the React Aria Button's API. These are not required for the implementation of `Button` but can be used to extend its functionality.
