...
The recommended pattern to achieve an icon only button (previously: `IconButton`) is to pass an icon in as props, then to use a [VisuallyHidden](/docs/components-visually-hidden--docs) component as `children`.
...
For an icon only `Button`, this can be paired with the `isPendingLabelHidden` prop to visibly hide the `pendingLabel` to avoid layout shifts. The `pendingLabel` should be short and declarative as this will be announced to a screen reader regardless of its visibility.
...
<ArgTypes of={exampleStories.Playground} exclude={["children", "className",  "size", "variant", "onPress", "icon", "iconPosition", "isFullWidth", "isDisabled", "isPending", "isPendingLabelHidden",  "pendingLabel"]} />
