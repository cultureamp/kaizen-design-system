...
For an icon only `Button`, this can be paired with the `hasHiddenPendingLabel` prop to visibly hide the `pendingLabel` to avoid layout shifts. The `pendingLabel` should be short and declarative as this will be announced to a screen reader regardless of its visibility.
