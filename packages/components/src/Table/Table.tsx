...
import { ExclamationIcon, SortAscendingIcon, SortDescendingIcon, } from "~components/Icon";
 ...
 */
export const TableContainer = ({ variant = "compact", children, ...otherProps }: TableContainerProps): JSX.Element => (<div role="table" className={classnames(styles.container, variant === "default" && styles.defaultSpacing, variant === "data" && styles.dataVariant)} {...otherProps}>
    {children}
  </div>);
export type TableHeaderProps = {
    children?: React.ReactNode;
};
export const TableHeader = ({ children, ...otherProps }: TableHeaderProps): JSX.Element => (<div role="rowgroup" {...otherProps}>
    {children}
  </div>);
export type TableHeaderRowProps = {
    children?: React.ReactNode;
};
const ratioToPercent = (width?: number): string | number | undefined => width != null ? `${width * 100}%` : width;
...
export type TableHeaderRowCellProps = {
    labelText: string;
    onClick?: ((e: React.MouseEvent<HTMLButtonElement>) => any) | ((e: React.MouseEvent<HTMLAnchorElement>) => any);
    width?: number;
    flex?: string;
    href?: string;
    icon?: ReactElement;
...
export const TableHeaderRowCell = ({ labelText, onClick, href, width, flex, icon, checkable, checkedStatus, checkboxLabel, onCheck, reversed, sorting: sortingRaw, 
...
wrapping = "nowrap", align = "start", tooltipInfo, isTooltipIconHidden = false, tooltipPortalSelector, sortingArrowsOnHover, classNameOverride, 
// There aren't any other props in the type definition, so I'm unsure why we
// have this spread.
...otherProps }: TableHeaderRowCellProps): JSX.Element => {
    ...
    let cellContents = (<div className={styles.headerRowCellLabelAndIcons}>
      {icon && (<span className={styles.headerRowCellIcon}>
          {cloneElement(icon, {
                title: labelText,
      ...
      {checkable && (<div className={styles.headerRowCellCheckbox}>
          <Checkbox checkedStatus={checkedStatus} onCheck={onCheck} aria-label={checkboxLabel}/>
        </div>)}
      {tooltipInfo != null && !isTooltipIconHidden ? (<div className={styles.headerRowCellTooltipIcon}>
          <ExclamationIcon role="presentation"/>
        </div>) : null}
      {/* If an "icon" is supplied, the label is displayed inside the icon aria title instead */}
      {!icon ? (<Heading tag="div" variant="heading-6" color={sorting || isHovered ? hoveredHeaderColor : headerColor} classNameOverride={styles.headerRowCellLabel}>
          {labelText}
        </Heading>) : null}
      {(sorting || (isHovered && sortingArrowsOnHover)) && (<div className={classnames(align === "center" && styles.headerRowCellIconAlignCenter, align === "end" && styles.headerRowCellIconAlignEnd, reversed && styles.whiteText)}>
          {sorting === "ascending" || sortingArrowsOnHover === "ascending" ? (<SortAscendingIcon role="presentation"/>) : (<SortDescendingIcon role="presentation"/>)}
        </div>)}
    </div>);
    cellContents = href ? (<a className={classnames(styles.headerRowCellButton, reversed && styles.headerRowCellButtonReversed)} href={href} onClick={onClick as (e: React.MouseEvent<HTMLAnchorElement>) => any | undefined} onMouseEnter={(): void => updateHoverState(true)} onFocus={(): void => updateHoverState(true)} onMouseLeave={(): void => updateHoverState(false)} onBlur={(): void => updateHoverState(false)}>
      {cellContents}
    </a>) : onClick ? (<button type="button" className={classnames(styles.headerRowCellButton, reversed && styles.headerRowCellButtonReversed)} onClick={onClick as (e: React.MouseEvent<HTMLButtonElement>) => any} onMouseEnter={(): void => updateHoverState(true)} onFocus={(): void => updateHoverState(true)} onMouseLeave={(): void => updateHoverState(false)} onBlur={(): void => updateHoverState(false)}>
      {cellContents}
    </button>) : (
    ...
    cellContents =
        tooltipInfo != null ? (<Tooltip animationDuration={0} classNameOverride={styles.headerRowCellTooltip} text={tooltipInfo} portalSelector={tooltipPortalSelector}>
        {cellContents}
      </Tooltip>) : (
    ...
    return (<div className={classnames(styles.headerRowCell, wrapping === "nowrap" && styles.headerRowCellNoWrap, align === "center" && styles.headerRowCellAlignCenter, align === "end" && styles.headerRowCellAlignEnd, sorting && styles.headerRowCellActive, classNameOverride)} style={{
            width: ratioToPercent(width),
            flex,
        }} role="columnheader" {...otherProps}>
      {cellContents}
    </div>);
};
...
export const TableCard = ({ children, expanded, expandedStyle = "well", onClick, href, forceHoverState = false, classNameOverride, ...otherProps }: TableCardProps): JSX.Element => {
    const className = classnames(styles.card, expanded && styles.expanded, expanded && styles[expandedStyle], (forceHoverState || onClick != null || href != null) &&
        styles.hasHoverState, classNameOverride);
    return href != null ? (<a href={href} className={className} onClick={onClick as AnchorClickEvent} {...otherProps}>
      {children}
    </a>) : onClick ? (<button type="button" className={className} onClick={onClick as ButtonClickEvent} {...otherProps}>
      {children}
    </button>) : (<div className={className} {...otherProps}>
      {children}
    </div>);
};
/**
...
export const TableRow = ({ children, classNameOverride, ...otherProps }: TableRowProps): JSX.Element => (<div className={classnames(styles.row, classNameOverride)} role="row" {...otherProps}>
    {children}
  </div>);
/**
 * @param width value between 1 and 0, to be calculated as a percentage
...
export const TableRowCell = ({ children, width, flex, href, classNameOverride, ...otherProps }: TableRowCellProps): JSX.Element => href != null ? (<a 
// eslint-disable-next-line jsx-a11y/no-interactive-element-to-noninteractive-role
role="cell" style={{
        width: ratioToPercent(width),
        flex,
    }} className={classnames(styles.rowCell, classNameOverride)} href={href} {...otherProps}>
      {children}
    </a>) : (<div role="cell" style={{
        width: ratioToPercent(width),
        flex,
    }} className={classnames(styles.rowCell, classNameOverride)} {...otherProps}>
      {children}
    </div>);
