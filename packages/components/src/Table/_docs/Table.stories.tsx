...
import { TableCard, TableCardProps, TableContainer, TableContainerProps, TableHeader, TableHeaderRowCell, TableHeaderRowCellProps, TableRow, TableRowCell, TableRowCellProps, TableRowProps, } from "../index";
...
const TableTemplate: StoryObj<TableStoryProps> = {
    render: ({ container, headerRowCells, tableCards, row, rowCells }) => (<TableContainer {...container}>
      <TableHeader>
        <TableRow>
          {headerRowCells.map((headerRowCellProps, index) => (<TableHeaderRowCell key={index} {...headerRowCellProps}/>))}
        </TableRow>
      </TableHeader>
      {tableCards.map((tableCardProps, index) => (<TableCard {...tableCardProps} key={index}>
          <TableRow {...row}>
            {rowCells.map(({ children, ...otherProps }, rowCellIndex) => (<TableRowCell {...otherProps} key={rowCellIndex}>
...
export default {
    ...
    decorators: [
        Story => (<div style={{ margin: "1rem auto", width: "100%", maxWidth: "60rem" }}>
        <Story />
      </div>),
    ],
