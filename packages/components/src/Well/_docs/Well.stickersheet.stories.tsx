...
const WellWrapped = (props: WellProps): JSX.Element => (<Well {...props}>
    <Text variant="body">
...
const StickerSheetTemplate: StickerSheetStory = {
    render: () => (<>
      <StickerSheet>
        <StickerSheet.Header headings={["Solid Border", "Dashed Border", "None"]} hasVerticalHeadings/>
        <StickerSheet.Body>
          {variantTypes.map(variant => (<StickerSheet.Row key={variant} rowTitle={variant}>
              {borderStyleTypes.map(border => (<WellWrapped key={border} variant={variant} borderStyle={border}/>))}
            </StickerSheet.Row>))}
        </StickerSheet.Body>
      </StickerSheet>
      <StickerSheet>
