...
To handle the AI loading state, use the `isAiLoading` prop. This will animate the Well's border and background while `true` and remain static while `false`.
...
To revert to a regular Well, set `isAiLoading` to `undefined`.
