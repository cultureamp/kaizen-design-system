...
import { ArrowLeftIcon, ArrowRightIcon, EmailIcon, FeedbackClassifyIcon, SecurityTipIcon, } from "~components/Icon";
import { BrandMomentCaptureIntro, BrandMomentPositiveOutro, BrandMomentError, AnimatedSceneProps, } from "~components/Illustration";
import { Text } from "~components/Text";
import { BrandMoment } from "../index";
import { MinimalBasic, MinimalCustomerFocused, FakeNavBar, } from "./ExampleHeaders";
const IS_CHROMATIC = isChromatic();
const illustrationProps = (IS_CHROMATIC ? {} : { isAnimated: true, loop: true }) satisfies AnimatedSceneProps;
const meta = {
...
const IconRTLTemplate: Pick<Story, "render" | "parameters"> = {
    render: ({ primaryAction, ...args }, { globals }) => (<BrandMoment {...args} primaryAction={{
            label: "Primary action label",
            ...primaryAction,
            icon: globals.textDirection === "ltr" ? (<ArrowRightIcon role="presentation"/>) : (<ArrowLeftIcon role="presentation"/>),
        }}/>),
    parameters: {
        docs: {
            source: {
