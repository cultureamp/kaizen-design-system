 ...
 */
export const BrandMoment = ({ mood, illustration, header, body, primaryAction, secondaryAction, text, classNameOverride, ...restProps }: BrandMomentProps): JSX.Element => {
    const { queries } = useMediaQueries();
    return (<div className={classnames(styles.body, styles[mood], classNameOverride)} {...restProps}>
          ...
          <div className={styles.mainInner}>
            ...
            <div className={styles.right}>
              <div className={styles.rightInner}>
                {text.subtitle && (<Heading variant="heading-3" tag="h1" classNameOverride={styles.subtitle}>
                    {text.subtitle}
                  </Heading>)}
                <Heading variant="display-0" tag={text.subtitle ? "h2" : "h1"} classNameOverride={styles.title}>
                  {text.title}
                </Heading>
                {text.body && (<Text variant="intro-lede" classNameOverride={styles.textBody}>
                    {text.body}
                  </Text>)}
                {body && <div className={styles.textBody}>{body}</div>}
                <div className={styles.actions}>
                  {primaryAction && (<Button primary fullWidth={queries.isSmall} {...primaryAction}/>)}
      ...
      {text.footer && (<footer className={styles.footer}>
          <div className={styles.container}>
