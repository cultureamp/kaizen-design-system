...
const GenericModalWrapper = ({ isOpen: propsIsOpen, onOutsideModalClick, onEscapeKeyup, ...props }: Partial<GenericModalProps>): JSX.Element => {
    const [isOpen, setIsOpen] = React.useState<boolean>(propsIsOpen ?? true);
    return (<GenericModal isOpen={isOpen} onOutsideModalClick={e => {
            setIsOpen(false);
            onOutsideModalClick?.(e);
        }} onEscapeKeyup={e => {
            setIsOpen(false);
            onEscapeKeyup?.(e);
        }} onAfterLeave={props.onAfterLeave} id="GenericModalTestId">
      <ModalHeader>
