...
export const GenericModal = ({ id: propsId, children, isOpen = true, focusLockDisabled, onEscapeKeyup, onOutsideModalClick, onAfterEnter, onAfterLeave: propsOnAfterLeave, }: GenericModalProps): JSX.Element => {
    const reactId = useId();
    ...
    return createPortal(<Transition appear={true} show={isOpen} enter={styles.animatingEnter} leave={styles.animatingLeave} beforeEnter={onBeforeEnterHandler} afterEnter={onAfterEnterHandler} afterLeave={onAfterLeaveHandler} data-generic-modal-transition-wrapper onClick={(e: React.MouseEvent): void => e.stopPropagation()}>
    ...
    autoFocus={false}>
        ...
        <div className={styles.scrollLayer} ref={(scrollLayerRef): void => {
            setScrollLayer(scrollLayerRef);
        }} onClick={outsideModalClickHandler} data-testid={`${id}-scrollLayer`}>
          <ModalContext.Provider value={{
            labelledByID,
            describedByID,
        }}>
            <div role="dialog" className={styles.modalLayer} aria-labelledby={labelledByID} aria-describedby={describedByID} ref={(modalLayerRef): void => setModalLayer(modalLayerRef)} data-testid={id}>
              {children}
            </div>
