...
const ContextModalWrapper = ({ children, ...props }: Partial<ContextModalProps>): JSX.Element => (<ContextModal isOpen={true} title="Example modal title" onConfirm={() => undefined} onDismiss={() => undefined} secondaryLabel="Example secondary" onSecondaryAction={() => undefined} {...props}>
    {children}
  </ContextModal>);
describe("<ContextModal />", () => {
    ...
    it("supports a dismiss action when dismiss button is pressed", async () => {
        ...
        const { getByLabelText } = render(<ContextModalWrapper onConfirm={handleConfirm} onDismiss={handleDismiss}>
        Example modal body
      </ContextModalWrapper>);
    ...
    it("supports a confirm action when confirm button is pressed", async () => {
        const handleConfirm = jest.fn();
        const handleDismiss = jest.fn();
        const { getByText } = render(<ContextModalWrapper onDismiss={handleDismiss} onConfirm={handleConfirm}>
        Example modal body
      </ContextModalWrapper>);
