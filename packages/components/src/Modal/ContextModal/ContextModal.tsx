...
import { GenericModal, ModalFooter, ModalHeader, ModalAccessibleLabel, ModalBody, } from "~components/Modal/GenericModal";
 ...
 */
export const ContextModal = ({ isOpen, unpadded = false, layout = "portrait", title, onConfirm, onDismiss: propsOnDismiss, onAfterLeave, onAfterEnter, confirmLabel = "Confirm", confirmWorking, renderBackground, children, contentHeader, image, secondaryLabel, onSecondaryAction, ...props }: ContextModalProps): JSX.Element => {
    ...
    return (<GenericModal isOpen={isOpen} onEscapeKeyup={onDismiss} onOutsideModalClick={onDismiss} onAfterLeave={onAfterLeave} onAfterEnter={onAfterEnter}>
        ...
        <ModalBody>
          <div className={classnames(styles.contentLayout, styles[`${layout}Contentlayout`])}>
            ...
            <div className={styles.content}>
              {children}
              {onConfirm != null && (<div className={secondaryLabel
                ? styles.footerWithSecondaryAction
                : styles.footer}></div>)}
            </div>
          </div>
        </ModalBody>
        <ModalFooter variant={image ? "context" : undefined} actions={footerActions} appearance="primary" unpadded={unpadded}/>
      </div>
    </GenericModal>);
};
