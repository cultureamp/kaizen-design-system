...
import { CautionWhiteIcon, ExclamationWhiteIcon, InformationWhiteIcon, SuccessWhiteIcon, } from "~components/Icon";
import { Assertive, Cautionary, Informative, Negative, Positive, } from "~components/Illustration";
import { GenericModal, ModalAccessibleDescription, ModalFooter, ModalHeader, ModalAccessibleLabel, ModalBody, } from "~components/Modal/GenericModal";
...
const getIcon = (mood: Mood, isProminent: boolean): JSX.Element => {
    switch (mood) {
        case "cautionary":
            return isProminent ? (<Cautionary />) : (<CautionWhiteIcon inheritSize role="presentation"/>);
        case "informative":
            return isProminent ? (<Informative />) : (<InformationWhiteIcon inheritSize role="presentation"/>);
        case "negative":
            return isProminent ? (<Negative />) : (<ExclamationWhiteIcon inheritSize role="presentation"/>);
        case "positive":
            return isProminent ? (<Positive />) : (<SuccessWhiteIcon inheritSize role="presentation"/>);
        case "assertive":
            return isProminent ? (<Assertive />) : (<ExclamationWhiteIcon inheritSize role="presentation"/>);
    }
};
/**
 ...
 */
export const ConfirmationModal = ({ isOpen, isProminent = false, unpadded = false, mood, title, onConfirm, onAfterLeave, onAfterEnter, confirmLabel = "Confirm", dismissLabel = "Cancel", confirmWorking, onDismiss: propsOnDismiss, children, ...props }: ConfirmationModalProps): JSX.Element => {
    ...
    return (<GenericModal isOpen={isOpen} onEscapeKeyup={onDismiss} onOutsideModalClick={onDismiss} onAfterLeave={onAfterLeave} onAfterEnter={onAfterEnter}>
      <div className={styles.modal} data-modal {...props}>
        <ModalHeader onDismiss={onDismiss}>
          <div className={classnames(styles.header, styles[`${mood}Header`], isProminent && styles.prominent, !unpadded && styles.padded)}>
        ...
        <ModalFooter actions={footerActions} appearance={mood === "negative" ? "destructive" : "primary"} unpadded={unpadded}/>
      </div>
    </GenericModal>);
};
