...
const InputEditModalWrapper = ({ children, ...props }: Partial<InputEditModalProps>): JSX.Element => (<InputEditModal isOpen={true} title="Example modal title" onSubmit={(): void => undefined} onDismiss={(): void => undefined} mood="positive" {...props}>
    {children}
  </InputEditModal>);
describe("<InputEditModal />", () => {
    ...
    it("supports a dismiss action when dismiss button is pressed", async () => {
        ...
        const { getByLabelText } = render(<InputEditModalWrapper onSubmit={handleSubmit} onDismiss={handleDismiss}>
        Example modal body
    ...
    it("supports a dismiss action when cancel button is pressed", async () => {
        const handleSubmit = jest.fn();
        const handleDismiss = jest.fn();
        const { getByText } = render(<InputEditModalWrapper onDismiss={handleDismiss} onSubmit={handleSubmit}>
        Example modal body
      </InputEditModalWrapper>);
