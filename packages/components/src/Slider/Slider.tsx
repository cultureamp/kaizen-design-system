 ...
 */
export const Slider = ({ id: propsId, labelText, description, labelPosition = "inline", variant = "default", disabled, readOnlyMessage, ...restProps }: SliderFieldProps): JSX.Element => {
    const id = propsId ?? useId();
    const descriptionId = `${id}-description`;
    return (<FieldGroup inline>
      <div className={classnames(styles.wrapper, labelPosition === "inline" && styles.labelInline)}>
        <div className={styles.labelWrapper}>
          ...
          {description && (<Text variant="small" id={descriptionId} classNameOverride={disabled ? styles.descriptionDisabled : undefined}>
              {description}
            </Text>)}
        </div>
