...
const BaseButtonWrapper = (customProps?: Partial<BaseButtonProps>): JSX.Element => <BaseButton label={BUTTON_LABEL} {...customProps}/>;
describe("<BaseButton />", () => {
    it("renders button as default", () => {
        render(<BaseButtonWrapper />);
        expect(screen.getByText(BUTTON_LABEL, { selector: "button" })).toBeInTheDocument();
    });
    it("renders anchor when href is defined", () => {
        render(<BaseButtonWrapper href=""/>);
        expect(screen.getByText(BUTTON_LABEL, { selector: "a" })).toBeInTheDocument();
    });
