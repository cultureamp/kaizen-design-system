...
export const useMediaQueries = (propQueries: Props = {}): {
    ...
    // ---------------------------------------
    const smallMatchMedia = useMemo(() => window.matchMedia(`(max-width: ${subtractOnePixel(breakpoints.medium)})`), [breakpoints.medium]);
    const mediumMatchMedia = useMemo(() => window.matchMedia(`(min-width: ${breakpoints.medium}) and (max-width: ${subtractOnePixel(breakpoints.large)})`), [breakpoints.large]);
    const largeMatchMedia = useMemo(() => window.matchMedia(`(min-width: ${breakpoints.large})`), [breakpoints.large]);
    ...
    useEffect(() => {
        ...
        const updateCustomMatches = (matchMedia: MediaQueryList, queryName: string): void => setCustomMatches({
            ...customQueryMatches,
    ...
    const kaizenComponents = {
        ...
        MediumOrSmaller: (props: HelperComponentProps) => (<>{kaizenMatches.isMediumOrSmaller && props.children}</>),
        MediumOrLarger: (props: HelperComponentProps) => (<>{kaizenMatches.isMediumOrLarger && props.children}</>),
    };
    // ---------------------------------------
    ...
    Object.keys(propQueries).map(key => {
        ...
        customComponents[componentName] = (props: HelperComponentProps): JSX.Element => <>{customMatches[key] && props.children}</>;
    });
    return {
