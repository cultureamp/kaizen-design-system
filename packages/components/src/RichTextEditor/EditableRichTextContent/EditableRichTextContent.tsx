...
const handleEditableClick = (e: MouseEvent<HTMLElement>, onClick: (event: MouseEvent<HTMLElement>) => void): void => {
    if ((e.target as HTMLElement).hasAttribute("href")) {
        return;
    }
    return onClick(e);
};
export const EditableRichTextContent = ({ onClick, content, classNameOverride, labelText, isLabelHidden = false, contentProps, ...restProps }: EditableRichTextContentProps): JSX.Element => (<>
    {!isLabelHidden && (<Label classNameOverride={styles.editorLabel} labelText={labelText}/>)}
    ...
    <div onClick={(e): void => handleEditableClick(e, onClick)} className={classnames(styles.editableContainer, classNameOverride)} {...restProps}>
      <VisuallyHidden classNameOverride={styles.hiddenButton}>
        <button type="button" onClick={onClick} aria-label={`Edit ${labelText}`}/>
      </VisuallyHidden>
