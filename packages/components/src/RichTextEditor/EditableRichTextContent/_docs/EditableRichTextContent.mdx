...
The `EditableRichTextContent` indicates interactivity similar to a `text` or `textarea` input and should be used in combination with the <LinkTo pageId="components-richtexteditor">RichTextEditor</LinkTo> to toggle between inactive and editable states.

This differs from the <LinkTo pageId="components-richtextcontent">RichTextContent</LinkTo> component, which is used to render `RichTextEditor` content as read-only text.
