...
import { ProseMirrorCommands, ProseMirrorModel, ProseMirrorSchemaList, ProseMirrorState, } from "../../utils/prosemirror";
...
const chainTransactions = (...commands: ProseMirrorState.Command[]): ProseMirrorState.Command => (state, dispatch): boolean => {
...
const createInitialParagraph = (state: ProseMirrorState.EditorState, dispatch?: (tr: ProseMirrorState.Transaction) => void): boolean => {
    if (dispatch) {
...
const createToggleMarkCommand = (mark: ProseMirrorModel.MarkType): ProseMirrorState.Command => (state, dispatch) => {
...
const createToggleListCommand = (node: ProseMirrorModel.NodeType): ProseMirrorState.Command => (state, dispatch) => {
    const docIsEmpty = state.doc.content.size === 0;
    if (docIsEmpty) {
...
const indentListIsDisabled = (state: ProseMirrorState.EditorState): boolean => {
    ...
    const range = $from.blockRange($to, node => node.childCount > 0 && node.firstChild!.type === listItemNode);
    return !range || range.startIndex === 0 ? true : false;
};
/** Creates an object used as an index to map the controls to respective groups */
const createControlGroupIndex = (controls: ToolbarItems[]): ControlGroupTypes => controls.reduce((groups, currentControl) => {
    if (!currentControl?.name)
        return groups;
    return {
        ...groups,
...
const createInitialControls = (controlGroupIndex: ControlGroupTypes): GroupedToolbarControls => {
    ...
    const initialControlObject: Record<string, ToolbarControl[]> = uniqueGroups.reduce((controlObject, controlKey) => {
        if (controlKey === "ungrouped")
...
const getGroupIndex = (controlGroupIndex: ControlGroupTypes, controlType?: ToolbarControlTypes): string => {
    if (controlType) {
...
const filterToolbarControls = (groupedControls: GroupedToolbarControls): ToolbarControl[][] => Object.values(groupedControls).filter(controls => controls.length > 0);
/** Builds an array of object used to map control configuration to rte toolbar buttons */
export const buildControlMap = (schema: ProseMirrorModel.Schema, editorState: ProseMirrorState.EditorState, controls?: ToolbarItems[]): ToolbarControl[][] => {
    if (!controls)
        return [];
    ...
    if (schema.nodes.orderedList || schema.nodes.bulletList) {
        const groupIndex = controlGroupIndex["orderedList"] ||
