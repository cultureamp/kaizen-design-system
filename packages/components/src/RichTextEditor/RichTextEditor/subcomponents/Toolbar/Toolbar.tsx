...
const handleKeyDown = (e: React.KeyboardEvent<HTMLElement>, buttonIndex: number, buttonFocusIndex: number, setFocusIndex: React.Dispatch<React.SetStateAction<number>>, toolbarButtons: React.MutableRefObject<any>): void => {
    if (!determineValidKeypress(e))
        return;
    let newFocusIndex;
...
export const Toolbar = (props: ToolbarProps): JSX.Element => {
    ...
    return (<div className={classnames(styles.toolbar, classNameOverride)} role="toolbar" {...toolbarProps}>
      {React.Children.map(toolbarChildren, (toolbarSection, sectionIndex) => {
            ...
            const { children: sectionChildren, ...toolbarSectionProps }: ToolbarSectionProps = toolbarSection.props;
            return (<toolbarSection.type {...toolbarSectionProps} key={`rte-section-${sectionIndex}`}>
            {React.Children.map(sectionChildren, toolbarButton => {
                    ...
                    buttonCount += 1;
                    return (<toolbarButton.type {...toolbarButton.props} key={`rte-button-${buttonIndex}`} tabIndex={buttonIndex === buttonFocusIndex ? 0 : -1} onKeyDown={(e: React.KeyboardEvent<HTMLElement>): void => handleKeyDown(e, buttonIndex, buttonFocusIndex, setButtonFocusIndex, toolbarButtonsRef)} ref={(ref: React.ReactNode | undefined): React.ReactNode => (toolbarButtonsRef.current[buttonIndex] = ref)}/>);
                })}
          </toolbarSection.type>);
        })}
    </div>);
