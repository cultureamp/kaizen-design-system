...
import { ProseMirrorCommands, ProseMirrorHistory, ProseMirrorKeymap, ProseMirrorModel, ProseMirrorState, } from "../utils/prosemirror";
 ...
 */
export const RichTextEditor = ({ id, onChange, defaultValue, labelText, "aria-labelledby": labelledBy, "aria-describedby": describedBy, classNameOverride, controls, rows = 3, dataError, onDataError, validationMessage, description, status = "default", ...restProps }: RichTextEditorProps): JSX.Element => {
    ...
    if (useRichTextEditorResult instanceof Error) {
        ...
        return (<InlineNotification headingProps={{
                children: "Error",
                variant: "heading-6",
            }} type="negative" persistent>
    ...
    return (<>
      {!labelledBy && labelText && (<Label classNameOverride={styles.editorLabel} id={labelId} labelText={labelText}/>)}
      <div className={classnames(styles.editorWrapper, styles[status])}>
        {controls && (<Toolbar aria-controls={editorId} aria-label="Text formatting" classNameOverride={styles.toolbar}>
            {controlMap.map((section, index) => (<ToolbarSection key={index}>
                {section.map((controlConfig, controlKeyIndex) => (<ToggleIconButton key={controlKeyIndex} icon={controlConfig.icon} disabled={controlConfig.disabled} label={controlConfig.label} isActive={controlConfig.isActive} onClick={(): void => dispatchTransaction(controlConfig.action)}/>))}
              </ToolbarSection>))}
          </Toolbar>)}
        <div id={editorId} ref={editorRef} className={classnames(styles.editor, styles[`rows${rows}`], classNameOverride, controls != null && controls.length > 0 && styles.hasToolbar)} {...restProps}/>
      </div>

      {validationMessage && (<FieldMessage id={validationMessageAria} message={validationMessage} status={status}/>)}

      {description && (<FieldMessage id={descriptionAria} message={description}/>)}
    </>);
};
RichTextEditor.displayName = "RichTextEditor";
function getPlugins(controls: ToolbarItems[] | undefined, schema: ProseMirrorModel.Schema): Array<ProseMirrorState.Plugin<unknown> | ProseMirrorState.Plugin<{
