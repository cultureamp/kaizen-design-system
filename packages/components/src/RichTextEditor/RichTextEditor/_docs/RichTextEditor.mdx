...
Whereas with all controls, the Kaizen `RichTextEditor` can create and render formatted text, lists, and links.
...
As mentioned in the previous section, the data passed to the `defaultValue` must be able to map to the `controls` provided, i.e: if your `defaultValue` contains bolded text, you must pass bold into your `controls`.
...
This will also throw if you have passed in an `EditorContentArray` that does match to the components `controls`.

<Canvas of={RichTextEditorStories.IncorrectDataForControls}/>

The `dataError` React Node and `onDataError` callback also allows you to handle these edge cases, while the entire page from breaking.
