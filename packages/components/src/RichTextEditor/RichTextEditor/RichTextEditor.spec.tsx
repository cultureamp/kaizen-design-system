...
const TestRTE = (args: Omit<RichTextEditorProps, "defaultValue" | "onChange" | "aria-labelledby"> & {
    ...
    const handleOnChange: RichTextEditorProps["onChange"] = (editorState): void => setRTEData(editorState.toJSON().doc.content);
    return (<RichTextEditor labelText="List RTE" rows={3} controls={[
            { name: "orderedList", group: "list" },
            { name: "bulletList", group: "list" },
        ]} defaultValue={rteData} onChange={handleOnChange} {...rest}/>);
};
describe("accessible name and description", () => {
    ...
    it("has the correct description with a description passed in, validation error, and aria-describedby", () => {
        render(<>
        <div id="external-description">External description</div>
        <TestRTE labelText="Some label" description="Some help text" validationMessage="Some error" aria-describedby="external-description"/>
      </>);
        expect(screen.getByRole("textbox", {
            name: "Some label",
