...
const TooltipContent = ({ position, text, referenceElement, tooltipId, mood = "default", }: TooltipContentProps): JSX.Element | null => {
    ...
    return isVisible || isAnimOut || isAnimIn ? (<div ref={setPopperElement} className={styles.tooltip} style={popperStyles.popper} {...attributes.popper} role="tooltip" id={tooltipId}>
...
const renderChildren = (content: ReactNode, tooltipId: string, hasActiveTooltip: boolean): ReactNode => {
...
export const Tooltip = ({ children, text, display = "block", position = "above", classNameOverride, portalSelector, animationDuration, isInitiallyVisible = false, mood = "default", }: TooltipProps): JSX.Element => {
    const [isHover, setIsHover] = useState(isInitiallyVisible);
    const [isFocus, setIsFocus] = useState(false);
    const [referenceElement, setReferenceElement] = useState<HTMLDivElement | null>(null);
    const tooltipId = useId();
    const hasActiveTooltip = isHover || isFocus;
    const tooltip = (<TooltipContent text={text} position={position} referenceElement={referenceElement} tooltipId={tooltipId} mood={mood}/>);
    ...
    return (<AnimationProvider isVisible={hasActiveTooltip} animationDuration={animationDuration}>
      <>
        <div ref={setReferenceElement} className={classnames(classNameOverride, getDisplayClassName())} onMouseEnter={(): void => setIsHover(true)} onMouseLeave={(): void => setIsHover(false)} onFocusCapture={(): void => setIsFocus(true)} onBlurCapture={(): void => setIsFocus(false)}>
