...
describe("isSemanticElement", () => {
    it("returns true if provided a native element with a semantic role", () => {
        expect(isSemanticElement(<button onClick={jest.fn()} type="button">
          click
        </button>)).toBe(true);
    ...
    it("will return true if provided a non-semantic element with a semantic role", () => {
        expect(isSemanticElement(<span tabIndex={0} role="button" onKeyDown={jest.fn} onClick={jest.fn()}>
          custom semantic el
        </span>)).toBe(true);
        expect(isSemanticElement(<div tabIndex={0} role="button" onKeyDown={jest.fn} onClick={jest.fn()}>
          custom semantic el
        </div>)).toBe(true);
        expect(isSemanticElement(<div role="textbox" contentEditable="true" aria-multiline="true"/>)).toBe(true);
    });
    it("returns false if provided an element using a role 'presentation' or 'none'", () => {
        expect(isSemanticElement(<ArrowRightIcon role="presentation"/>)).toBe(false);
        expect(isSemanticElement(<span role="none">
