...
import { Tooltip as RACTooltip, TooltipContext, TooltipProps as RACTooltipProps, TooltipTriggerStateContext, useContextProps, } from "react-aria-components";
 ...
 */
export const Tooltip = forwardRef<HTMLDivElement, TooltipProps>(({ children, className, ...props }, ref): JSX.Element => {
    ...
    useLayoutEffect(() => {
        setShouldInlineHiddenContent(!!triggerRef?.current?.getAttribute("data-inline-hidden-content"));
    }, [triggerRef]);
    return (<>
        <RACTooltip ref={ref} offset={8} {...props} className={mergeClassNames(styles.tooltip, className, reverseColors && styles.reversed)}>
          {renderProps => (<>
              <OverlayArrow />
        ...
        {shouldInlineHiddenContent ? (<VisuallyHidden>
            {typeof children === "function"
                ? children({
                    placement: "center",
                    isEntering: false,
