...
export const DateInputDescription = ({ description, locale, }: DateInputDescriptionProps): JSX.Element => {
    const { formatMessage } = useIntl();
    const hasCustomDescription = (typeof description === "string" && description !== "") ||
    ...
    if (hasCustomDescription) {
        return (<span className={styles.dateInputDescription}>
        <span>{description}</span>
        <span className={styles.dateInputFormatContainer}>
          (
          <LabelledMessage label={translatedInputFormatMessage} message={formatDescriptionInputFormat(locale)}/>
          )
        </span>
      </span>);
    }
    return (<LabelledMessage label={translatedInputFormatMessage} message={formatDescriptionInputFormat(locale)}/>);
};
