...
const CalendarPopoverExample = ({ children, rowHeight = 300, 
/** this is here as a convenient way to test overlap */
strategy = "fixed", }: Partial<CalendarPopoverProps & {
    rowHeight: number;
    /** this is here as a convenient way to test overlap */
    ...
    return (<>
      <div className="bg-orange-300 inline-block" ref={setReferenceElement} style={{ marginBottom: `${rowHeight}px` }}>
        Reference element
      </div>
      <CalendarPopover referenceElement={referenceElement} floatingOptions={{
            strategy,
            middleware: [
                offset(15),
                size({
