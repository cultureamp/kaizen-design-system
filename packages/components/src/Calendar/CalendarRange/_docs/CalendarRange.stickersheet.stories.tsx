...
const CalendarRangeWrapper = (props: Partial<CalendarRangeProps>): JSX.Element => (<div data-testid={props.id}>
...
const applyStickerSheetStyles = (canvasElement: HTMLElement): void => {
    ...
    const getElementWithinCalendar = (id: string, monthName: string, dayName: string): HTMLElement => {
    ...
    calendarsPseudoStates.forEach(({ id, monthName, dayName }) => {
        getElementWithinCalendar(`${id}--hover`, monthName, dayName).setAttribute("data-sb-pseudo-styles", "hover");
        getElementWithinCalendar(`${id}--focus`, monthName, dayName).setAttribute("data-sb-pseudo-styles", "focus");
    });
};
