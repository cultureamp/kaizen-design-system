...
The `variant` prop replaces the `mood` prop.

All moods have been replaced by the `default` variant, with the exception of the `prominent` mood,
which has been replaced by the `expert-advice` variant.
