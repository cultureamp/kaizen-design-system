...
import { mapArrowVariantToClass, mapLineVariant, mapSizeToClass, mapVariantToBoxClass, mapVariantToIcon, mapVariantToIconClass, } from "./utils/classMappers";
 ...
 */
export const Popover = ({ children, variant = "default", placement = "top", size = "small", heading, dismissible = false, onClose, singleLine = false, customIcon, referenceElement, classNameOverride, ...restProps }: PopoverProps): JSX.Element => {
    ...
    return (<div ref={setPopperElement} style={popperStyles.popper} {...attributes.popper} className={classnames(styles.root, mapSizeToClass(size), classNameOverride)} {...restProps}>
      <div className={mapVariantToBoxClass(variant)}>
        {heading && (<div className={styles.header}>
            {variant !== "default" && (<span className={classnames(styles.icon, mapVariantToIconClass(variant))}>
                {customIcon ?? mapVariantToIcon(variant)}
              </span>)}
            <Heading variant="heading-6" classNameOverride={styles.singleLine}>
              {heading}
            </Heading>
            {dismissible && (<button className={styles.close} onClick={onClose} type="button" aria-label="close">
                <CloseIcon role="presentation"/>
              </button>)}
          </div>)}
        <Text tag="div" variant="small" classNameOverride={classnames(styles.container, mapLineVariant(singleLine))}>
          {children}
        </Text>
      </div>
...
export const usePopover = (): [
    ...
    () => (props: PopoverPropsWithoutRef) => referenceElement ? (<Popover {...props} referenceElement={referenceElement}/>) : null, [referenceElement]);
