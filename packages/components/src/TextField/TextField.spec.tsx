...
describe("<TextField />", () => {
    ...
    it("renders correct aria-describedby when only validation message provided", () => {
        const { getByRole } = render(<TextField {...defaultProps} description={undefined} validationMessage="Error message"/>);
        const input = getByRole("textbox", {
    ...
    it("renders correct aria-describedby when both description and validation message provided", () => {
        const { getByRole } = render(<TextField {...defaultProps} description="Description text" validationMessage="Error message"/>);
        const input = getByRole("textbox", {
    ...
    it("renders empty aria-describedby when no description or validation message provided", () => {
        const { getByRole } = render(<TextField {...defaultProps} description={undefined} validationMessage={undefined}/>);
        const input = getByRole("textbox", {
            description: "",
        });
        expect(input).toBeInTheDocument();
    ...
    it("renders a TextField with the correct input type", () => {
        const { getByLabelText } = render(<TextField {...defaultProps} labelText="Password input" type="password"/>);
