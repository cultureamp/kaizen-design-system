 ...
 */
export const ProgressBar = ({ value, max, isAnimating, mood, subtext, label, classNameOverride, isReversed = false, ...restProps }: ProgressBarProps): JSX.Element => {
    const percentage = calculatePercentage({ value, max });
    return (<div role="progressbar" aria-valuenow={percentage} aria-valuemin={0} aria-valuemax={100} className={classNameOverride} {...restProps}>
      {label && <Label content={label} isReversed={isReversed}/>}
      <div className={styles.progressBackground}>
        <div className={classnames(styles[mood], isAnimating && styles.isAnimating)} style={{ transform: `translateX(-${100 - percentage}%` }}/>
      </div>
      {subtext && (<div className={styles.subtext}>
