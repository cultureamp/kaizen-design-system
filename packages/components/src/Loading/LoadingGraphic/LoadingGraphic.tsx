 ...
 */
export const LoadingGraphic = ({ isAnimated, isReversed, size, classNameOverride, ...props }: LoadingGraphicProps): JSX.Element => {
    const sizeValue = `${loadingGraphicSizeMap.get(size)}rem`;
    return (<div className={classnames(styles.loadingGraphic, isAnimated && styles.isAnimated, isReversed && styles.isReversed, classNameOverride)} style={{ width: sizeValue, height: sizeValue }} {...props}/>);
};
