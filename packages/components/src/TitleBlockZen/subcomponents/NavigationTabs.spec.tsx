...
const CustomComponent = (props: CustomNavigationTabProps): JSX.Element => (<button type="button" onClick={props.handleClick} className={props.className}>{`${props.href} - ${props.text} - ${props.active}`}</button>);
describe("NavigationTabs", () => {
    ...
    describe("with a render prop", () => {
        it("renders the component passed with the navigation tab props", async () => {
            ...
            const href = "www.cultureamp.com";
            render(<NavigationTab text={text} href={href} handleClick={handleClick} active render={CustomComponent} variant="education"/>);
