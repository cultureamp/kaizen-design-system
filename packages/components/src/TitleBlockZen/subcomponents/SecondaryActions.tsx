...
const renderSecondaryOverflowMenu = (secondaryOverflowMenuItems?: TitleBlockMenuItemProps[], reversed?: boolean): JSX.Element | undefined => {
    if (!secondaryOverflowMenuItems)
        return undefined;
    return (<Menu align="right" button={<IconButton label="Open secondary menu" reversed={reversed} icon={<MeatballsIcon role="presentation"/>} id={TITLE_BLOCK_ZEN_SECONDARY_MENU_HTML_ID}/>}>
      <MenuList>
        {secondaryOverflowMenuItems.map((menuItem, i) => (<TitleBlockMenuItem key={i} {...menuItem}/>))}
      </MenuList>
    </Menu>);
};
...
export const SecondaryActions = ({ secondaryActions, secondaryOverflowMenuItems, reversed = false, }: Props): JSX.Element | null => {
    if (!secondaryActions && !secondaryOverflowMenuItems)
        return null;
        ...
        ? secondaryActions.map((action, i) => {
            if ("menuItems" in action) {
                return {
                    ...
                    node: (<Menu align="right" button={<Button secondary label={action.label} reversed={reversed} icon={<ChevronDownIcon role="presentation"/>} iconPosition="end"/>}>
                <MenuList>
                  {action.menuItems.map((menuItem, i2) => (<TitleBlockMenuItem key={i2} {...menuItem}/>))}
                </MenuList>
              </Menu>),
                };
            }
                ...
                return {
                    key: `${i}`, // We shouldn't use an index here, see note above
                    node: (<Button secondary reversed={reversed} {...action} data-automation-id="title-block-secondary-actions-button" data-testid="title-block-secondary-actions-button"/>),
                };
            }
        })
        : [];
    ...
    return (<div className={styles.secondaryActionsContainer}>
