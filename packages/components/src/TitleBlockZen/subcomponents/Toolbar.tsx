...
export const Toolbar = ({ items, noGap = false, automationId, }: ToolbarProps): JSX.Element => {
    ...
    return (<div className={styles.toolbar} data-automation-id={automationId} data-testid={automationId}>
      {items.map(item => (<div className={classnames(styles.toolbarItem, noGap && styles.noGap)} key={item.key}>
          {item.node}
        </div>))}
    </div>);
};
