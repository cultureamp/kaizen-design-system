...
const inputDateOnSubmit = jest.fn<void, [
    Date | undefined
]>();
const FilterDatePickerFieldWrapper = ({ selectedDate, ...restProps }: Partial<FilterDatePickerFieldProps>): JSX.Element => {
    const [selectedValue, setSelectedValue] = useState<Date | undefined>(selectedDate);
    return (<FilterDatePickerField id="test__filter-date-picker" selectedDate={selectedValue} onDateChange={setSelectedValue} onDateSubmit={inputDateOnSubmit} locale="en-AU" {...restProps}/>);
};
...
describe("<FilterDatePickerField />", () => {
    ...
    describe("Inputs", () => {
        ...
        it("pre-fills the inputs when date is provided", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-01")}/>);
            await waitForI18nContent();
            const inputDate = screen.getByLabelText("Date");
            expect(inputDate).toHaveValue("1 May 2022");
        });
        it("allows customising the input labels", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-01")} inputProps={{
                    labelText: "Due date",
                }}/>);
            await waitFor(() => {
        ...
        describe("onBlur", () => {
            it("updates date input and calendar values correctly on blur", async () => {
                const inputDateOnBlur = jest.fn<void, [
                    FocusEvent
                ]>();
                renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-02")} onBlur={inputDateOnBlur}/>);
                await waitForI18nContent();
                const inputDate = screen.getByLabelText("Date");
        ...
        describe("Press Enter key", () => {
            it("updates calendar values and calls submit when the date is valid", async () => {
                renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-02")}/>);
                await waitForI18nContent();
                const inputDate = screen.getByLabelText("Date");
                expect(inputDate).toHaveValue("2 May 2022");
                const targetMonth = screen.getByRole("grid", { name: "May 2022" });
    ...
    describe("Calendar", () => {
        it("shows the default month as the start month when there isn't a selected value", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper defaultMonth={new Date("2022-05-02")}/>);
            await waitForI18nContent();
            expect(screen.getByText("May 2022")).toBeVisible();
        });
        ...
        it("shows the current month as the start month when selected date is invalid", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("potato")}/>);
            await waitForI18nContent();
        ...
        it("calls the onDateSubmit when selecting a date", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper defaultMonth={new Date("2022-05-01")}/>);
            const targetMonth = screen.getByRole("grid", { name: "May 2022" });
            const targetDay = within(targetMonth).getByRole("gridcell", {
                name: "15",
            });
        ...
        it("updates the input when changing the date", async () => {
            renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-15")}/>);
            await waitForI18nContent();
            const inputDate = screen.getByLabelText("Date");
    ...
    describe("Validation", () => {
        ...
        describe("Custom validation", () => {
            it("shows validation messages passed in from the consumer", async () => {
                renderWithIntl(<FilterDatePickerFieldWrapper onValidate={(): void => undefined} validationMessage={{
                        status: "error",
        ...
        describe("Inbuilt validation", () => {
            ...
            it("shows inbuilt validation messages for pre-existing values", async () => {
                const { container } = renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("2022-05-15")} disabledDays={[new Date("2022-05-15")]}/>);
                await waitForI18nContent();
                const dateErrorContainer = container.querySelector(dateErrorId);
                expect(dateErrorContainer).toHaveTextContent("15/05/2022 is not available, try another date");
            });
        });
        it("does not call onDateSubmit when the input value is invalid", async () => {
            const { getByLabelText } = renderWithIntl(<FilterDatePickerFieldWrapper />);
            await waitForI18nContent();
        ...
        it("re-validates values when selecting a value using the calendar", async () => {
            const { container } = renderWithIntl(<FilterDatePickerFieldWrapper selectedDate={new Date("potato")} defaultMonth={new Date("2022-05-01")}/>);
            const dateErrorContainer = container.querySelector(dateErrorId);
            await waitFor(() => {
