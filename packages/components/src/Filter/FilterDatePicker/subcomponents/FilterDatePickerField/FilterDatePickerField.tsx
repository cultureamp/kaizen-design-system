...
import { CalendarSingle, CalendarSingleProps, DisabledDays, isInvalidDate, } from "~components/Calendar";
...
export const FilterDatePickerField = ({ id: propsId, inputProps, locale: propsLocale, defaultMonth, selectedDate, onDateChange, onDateSubmit, disabledDays, description, validationMessage, onValidate, classNameOverride, ...restProps }: FilterDatePickerFieldProps): JSX.Element => {
    const reactId = useId();
    ...
    const [state, dispatch] = useReducer(filterDatePickerFieldReducer, {
        ...
        startMonth: selectedDate && !isInvalidDate(selectedDate)
            ? selectedDate
    ...
    return (<div className={classnames(styles.filterDatePickerField, classNameOverride)} {...restProps}>
      <DateInputField id={`${id}--input`} locale={locale} value={state.inputValue} description={description} validationMessage={dateValidation.validationMessage} {...inputDateHandlers} {...inputProps}/>
      <CalendarSingle disabled={disabledDays} locale={locale} selected={state.selectedDate} onSelect={handleCalendarSelect} month={state.startMonth} onMonthChange={(value: Date) => dispatch({ type: "navigate_months", date: value })}/>
    </div>);
};
