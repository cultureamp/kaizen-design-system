...
export const getDateValidationHandler = ({ onValidate, setInbuiltValidationMessage, inputLabel, }: GetDateValidationHandlerArgs) => (validationResponse: DateValidationResponse): void => {
    if (onValidate)
    ...
    setInbuiltValidationMessage({
        ...
        message: inputLabel ? (<LabelledMessage label={`${getNodeText(inputLabel)}`} message={validationMessage.message}/>) : (validationMessage.message),
    });
};
