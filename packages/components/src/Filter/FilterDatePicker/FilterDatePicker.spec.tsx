...
const FilterDatePickerWrapper = ({ selectedDate, ...restProps }: Partial<FilterDatePickerProps>): JSX.Element => {
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const [selectedValue, setSelectedValue] = useState<Date | undefined>(selectedDate);
    return (<FilterDatePicker id="test__filter-date-picker" isOpen={isOpen} setIsOpen={setIsOpen} renderTrigger={(triggerProps): JSX.Element => (<FilterButton {...triggerProps}/>)} label="Drank" selectedDate={selectedValue} onDateChange={setSelectedValue} defaultMonth={new Date("2022-05-01")} locale="en-AU" {...restProps}/>);
};
describe("<FilterDatePicker />", () => {
    ...
    describe("Filter button", () => {
        it("should show the selected date in the button", async () => {
            renderWithIntl(<FilterDatePickerWrapper selectedDate={new Date("2022-05-01")}/>);
            await waitFor(() => {
    ...
    describe("Text input", () => {
        it("validates the date on blur", async () => {
            const { getByRole, getByLabelText, getByText } = renderWithIntl(<FilterDatePickerWrapper />);
            const triggerButton = getByRole("button", {
                name: "Drank",
            });
            await user.click(triggerButton);
        ...
        describe("Pressing Enter in the text input", () => {
            it("closes the popover when a valid date has been submitted via the text input field", async () => {
                const { getByRole, getByLabelText } = renderWithIntl(<FilterDatePickerWrapper />);
                const triggerButton = getByRole("button", {
                    name: "Drank",
                });
                await user.click(triggerButton);
    ...
    it("does not close the popover when the text input field has been cleared", async () => {
        const { getByRole, getByLabelText, getByText, queryByText } = renderWithIntl(<FilterDatePickerWrapper selectedDate={new Date("32/13/2022")}/>);
        const triggerButton = getByRole("button", {
        ...
        await user.tab();
        await waitFor(() => {
            expect(queryByText("Invalid Date is an invalid date")).not.toBeInTheDocument();
    ...
    it("does not close the popover when there is a selected date and the user navigates months", async () => {
        const { getByRole } = renderWithIntl(<FilterDatePickerWrapper selectedDate={new Date("01/01/2022")}/>);
        const triggerButton = getByRole("button", {
    ...
    it("updates the selected value in the trigger button when typing a date", async () => {
        const { getByRole, getByLabelText } = renderWithIntl(<FilterDatePickerWrapper selectedDate={new Date("06-06-2022")}/>);
        const triggerButton = getByRole("button", {
        ...
        await waitFor(() => {
            expect(getByRole("button", { name: "Drank : 7 Jun 2022" })).toBeInTheDocument();
        });
    });
});
