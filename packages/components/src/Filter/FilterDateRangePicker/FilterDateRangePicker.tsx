...
import { FilterDateRangePickerField, FilterDateRangePickerFieldProps, } from "./subcomponents/FilterDateRangePickerField";
...
export const FilterDateRangePicker = ({ id: propsId, isOpen, setIsOpen, renderTrigger, selectedRange, label, locale, ...restProps }: FilterDateRangePickerProps): JSX.Element => {
    const reactId = useId();
    const id = propsId ?? reactId;
    return (<Filter isOpen={isOpen} setIsOpen={setIsOpen} renderTrigger={(triggerProps): JSX.Element => renderTrigger({
            selectedValue: isCompleteDateRange(selectedRange) &&
                isValidRange(selectedRange.from, selectedRange.to) ? (<DateRangeDisplayLabel dateRange={selectedRange} locale={getLocale(locale)}/>) : undefined,
            label,
            ...triggerProps,
        })}>
      <FilterContents>
        <FilterDateRangePickerField id={`${id}--input`} label={label} locale={locale} selectedRange={selectedRange} {...restProps}/>
      </FilterContents>
    </Filter>);
};
