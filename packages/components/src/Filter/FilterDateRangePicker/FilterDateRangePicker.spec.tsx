...
const FilterDateRangePickerWrapper = ({ selectedRange, ...restProps }: Partial<FilterDateRangePickerProps>): JSX.Element => {
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const [selectedDateRange, setSelectedDateRange] = useState<DateRange | undefined>(selectedRange);
    return (<FilterDateRangePicker id="test__filter-date-range-picker" isOpen={isOpen} setIsOpen={setIsOpen} renderTrigger={(triggerProps): JSX.Element => (<FilterButton {...triggerProps}/>)} label="Dates" selectedRange={selectedDateRange} onRangeChange={setSelectedDateRange} locale="en-AU" {...restProps}/>);
};
describe("<FilterDateRangePicker />", () => {
    ...
    describe("Filter button", () => {
        it("should show the selected range in the button", () => {
            const { getByRole } = render(<FilterDateRangePickerWrapper selectedRange={{
        ...
        it("should show the calendar when the filter button is clicked", async () => {
            const { queryByRole, getByRole, getByText } = renderWithIntl(<FilterDateRangePickerWrapper defaultMonth={new Date("2022-05-01")}/>);
            expect(queryByRole("dialog")).not.toBeInTheDocument();
            const filterButton = getByRole("button", { name: "Dates" });
        ...
        it("should not show a date range in the button if the selected range is not valid", async () => {
            const { getByRole, getByLabelText } = renderWithIntl(<FilterDateRangePickerWrapper selectedRange={{ from: new Date("2022-05-01") }}/>);
            const filterButton = getByRole("button", { name: "Dates" });
            await user.click(filterButton);
            await waitFor(() => {
