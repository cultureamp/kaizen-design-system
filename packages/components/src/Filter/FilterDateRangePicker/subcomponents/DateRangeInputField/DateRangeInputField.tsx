...
import { DateInput, DateInputDescription, DateInputDescriptionProps, DateInputProps, } from "~components/DateInput";
import { FieldMessage } from "~components/FieldMessage";
import { VisuallyHidden } from "~components/VisuallyHidden";
import { OverrideClassName } from "~types/OverrideClassName";
import { isRefObject } from "~utils/isRefObject";
import { DateRangeValidationMessage, DateRangeValidationMessageProps, } from "../DateRangeValidationMessage";
...
export const DateRangeInputField = React.forwardRef<DateRangeInputFieldRefs, DateRangeInputFieldProps>(({ id, legend, inputStartDateProps, inputEndDateProps, description, disabled, isReversed = false, validationMessage, locale, classNameOverride, ...restProps }, ref) => {
    ...
    return (<div className={classNameOverride} {...restProps}>
        <fieldset className={styles.dateRangeInputContainer}>
          <legend>
            <VisuallyHidden>{legend}</VisuallyHidden>
          </legend>
          <DateInput ref={inputStartDateRef} id={`${id}--from`} aria-describedby={dateStartInputDescribedBy} aria-errormessage={dateStartErrorMessageId} aria-invalid={dateStartIsInvalid} autoComplete="off" disabled={disabled} status={validationMessage?.dateStart?.status} {...inputStartDateProps} classNameOverride={classnames(styles.inputStartDate, inputStartDateProps.classNameOverride)}/>
          <DateInput ref={inputEndDateRef} id={`${id}--to`} aria-describedby={dateEndInputDescribedBy} aria-errormessage={dateEndErrorMessageId} aria-invalid={dateEndIsInvalid} autoComplete="off" disabled={disabled} status={validationMessage?.dateEnd?.status} {...inputEndDateProps} classNameOverride={classnames(styles.inputEndDate, inputEndDateProps.classNameOverride)}/>
        </fieldset>

        {validationMessage && (<DateRangeValidationMessage validationMessage={validationMessage} isReversed={isReversed} dateStartId={dateStartErrorMessageId} dateEndId={dateEndErrorMessageId}/>)}
