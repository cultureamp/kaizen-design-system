...
const DateRangeInputFieldWrapper = (props: Partial<DateRangeInputFieldProps>): JSX.Element => (<DateRangeInputField id="test__date-range-input-field" legend="Dates" inputStartDateProps={{ labelText: "Date from" }} inputEndDateProps={{ labelText: "Date to" }} locale={enAU} {...props}/>);
describe("<DateRangeInputField />", () => {
    it("has unique ids for both inputs", async () => {
        const { container } = renderWithIntl(<DateRangeInputFieldWrapper id="range"/>);
    ...
    it("adds validation message to description if it exists", async () => {
        renderWithIntl(<DateRangeInputFieldWrapper validationMessage={{
                dateStart: {
                    status: "error",
        ...
        const inputStart = screen.getByRole("textbox", { name: "Date from" });
        await waitFor(() => {
            expect(inputStart).toHaveAccessibleDescription("Date Start has an error Input format : dd/mm/yyyy");
    ...
    describe("Refs", () => {
        it("correctly passes through both input refs", async () => {
            ...
            const Wrapper = (): JSX.Element => {
                ...
                const handleClick = (): void => onButtonClick(inputStartDateRef.current?.id, inputEndDateRef.current?.id);
                return (<>
            <DateRangeInputField ref={ref} id="test__id" legend="Dates" inputStartDateProps={{ labelText: "Start" }} inputEndDateProps={{ labelText: "End" }} locale={enAU}/>
