...
const FilterDateRangePickerFieldWrapper = ({ selectedRange, ...restProps }: Partial<FilterDateRangePickerFieldProps>): JSX.Element => {
    const [selectedDateRange, setSelectedDateRange] = useState<DateRange | undefined>(selectedRange);
    return (<FilterDateRangePickerField id="test__filter-date-range-picker" label="Dates" selectedRange={selectedDateRange} onRangeChange={setSelectedDateRange} locale="en-AU" {...restProps}/>);
};
...
describe("<FilterDateRangePickerField />", () => {
    describe("Inputs", () => {
        ...
        it("pre-fills the inputs when date range is provided", async () => {
            renderWithIntl(<FilterDateRangePickerFieldWrapper selectedRange={{
        ...
        it("allows customising the input labels", async () => {
            renderWithIntl(<FilterDateRangePickerFieldWrapper selectedRange={{
                    from: new Date("2022-05-01"),
                    to: new Date("2022-05-22"),
                }} inputStartDateProps={{ labelText: "Start date" }} inputEndDateProps={{ labelText: "End date" }}/>);
            await waitFor(() => {
        ...
        describe("onBlur", () => {
            it("updates start date input and calendar values correctly on blur", async () => {
                ...
                renderWithIntl(<FilterDateRangePickerFieldWrapper selectedRange={{
                        ...
                        to: new Date("2022-05-22"),
                    }} inputStartDateProps={{ onBlur: startDateOnBlur }} inputEndDateProps={{ onBlur: endDateOnBlur }}/>);
                await waitForI18nContent();
                const inputStartDate = screen.getByLabelText("Date from");
                const inputEndDate = screen.getByLabelText("Date to");
    ...
    describe("Calendar", () => {
        it("shows the default month as the start month when there isn't a selected value", async () => {
            renderWithIntl(<FilterDateRangePickerFieldWrapper defaultMonth={new Date("2022-05-02")}/>);
            await waitForI18nContent();
            expect(screen.getByText("May 2022")).toBeVisible();
            expect(screen.getByText("June 2022")).toBeVisible();
        });
    ...
    describe("Validation", () => {
        ...
        describe("Custom validation", () => {
            it("shows validation messages passed in from the consumer", async () => {
                renderWithIntl(<FilterDateRangePickerFieldWrapper onValidate={{
        ...
        describe("Inbuilt validation", () => {
            it("shows inbuilt validation messages for start date", async () => {
                const { container } = renderWithIntl(<FilterDateRangePickerFieldWrapper inputStartDateProps={{ labelText: "Start date" }}/>);
                await waitFor(() => {
            ...
            it("shows inbuilt validation messages for end date", async () => {
                const { container } = renderWithIntl(<FilterDateRangePickerFieldWrapper inputEndDateProps={{ labelText: "End date" }}/>);
                await waitForI18nContent();
                const inputEndDate = screen.getByLabelText("End date");
                await user.clear(inputEndDate);
            ...
            it("shows inbuilt validation messages for pre-existing values", async () => {
                const { container } = renderWithIntl(<FilterDateRangePickerFieldWrapper selectedRange={{
                        from: new Date("2022-05-15"),
                        to: new Date("2022-05-23"),
                    }} disabledDays={[new Date("2022-05-15"), new Date("2022-05-23")]}/>);
                await waitForI18nContent();
                const dateStartErrorContainer = container.querySelector(dateStartErrorId);
                expect(dateStartErrorContainer).toHaveTextContent("Date from:15/05/2022 is not available, try another date");
                const dateEndErrorContainer = container.querySelector(dateEndErrorId);
                expect(dateEndErrorContainer).toHaveTextContent("Date to:23/05/2022 is not available, try another date");
            });
            describe("Validates end date is not before start date", () => {
                const invalidDateOrderErrorMessage = 'Date to:Cannot be earlier than the selection in "Date from"';
        ...
        describe("Combined validation", () => {
            it("shows custom start date validation with inbuilt end date validation", async () => {
                const { container } = renderWithIntl(<FilterDateRangePickerFieldWrapper onValidate={{
                        dateStart: (): void => undefined,
                    }} validationMessage={{
                        dateStart: {
                            status: "error",
    ...
    it("only returns a valid date to the onRangeChange function", async () => {
        ...
        const { getByLabelText } = renderWithIntl(<FilterDateRangePickerFieldWrapper selectedRange={{
                from: new Date("2022-05-10"),
