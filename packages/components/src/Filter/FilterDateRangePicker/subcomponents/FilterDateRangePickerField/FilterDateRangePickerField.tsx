...
import { CalendarRange, CalendarRangeProps, DateRange, DisabledDays, formatDateAsText, isInvalidDate, parseDateFromTextFormatValue, } from "~components/Calendar";
...
export const FilterDateRangePickerField = ({ id, label, locale: propsLocale, defaultMonth, selectedRange, onRangeChange, disabledDays, inputStartDateProps, inputEndDateProps, description, validationMessage, onValidate, classNameOverride, ...restProps }: FilterDateRangePickerFieldProps): JSX.Element => {
    ...
    const transformDateToInputValue = (date: Date | undefined): string => date ? formatDateAsText(date, disabledDays, locale) : "";
    ...
    const validateStartDate = (date: Date | undefined, inputValue: string): Date | undefined => dateStartValidation.validateDate({
        date,
        inputValue,
    });
    const validateEndDate = (date: Date | undefined, inputValue: string): Date | undefined => dateEndValidation.validateDate({
        endDate: date,
    ...
    const [state, dispatch] = useReducer(filterDatePickerFieldReducer, {
        ...
        startMonth: selectedRange?.from && !isInvalidDate(selectedRange.from)
            ? selectedRange.from
    ...
    return (<div className={classnames(styles.filterDateRangePickerField, classNameOverride)} {...restProps}>
      <DateRangeInputField id={`${id}--input`} legend={label} inputStartDateProps={{
        ...
        }} locale={locale} description={description} validationMessage={{
      ...
      <CalendarRange disabled={disabledDays} locale={locale} selected={{
            from: state.selectedStartDate,
            to: state.selectedEndDate,
        }} onSelect={handleCalendarSelectRange} month={state.startMonth} onMonthChange={(value: Date) => dispatch({ type: "navigate_months", date: value })}/>
    </div>);
};
