...
export const MultiSelectOption = ({ classNameOverride, item, }: MultiSelectOptionProps): JSX.Element => {
    ...
    return (<li {...mergeProps(optionProps, focusProps)} ref={ref} className={classnames(styles.option, classNameOverride, isSelected && styles.isSelected, isFocusVisible && styles.isFocusVisible, isDisabled && styles.isDisabled)} aria-label={item.value?.label} aria-describedby={item.value?.count ? countElementId : undefined}>
      ...
      {item.rendered}
      {item.value?.count && (<span id={countElementId} className={styles.badgeContainer}>
