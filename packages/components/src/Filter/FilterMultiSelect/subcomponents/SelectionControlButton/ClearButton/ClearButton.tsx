...
export const ClearButton = (): JSX.Element => {
    ...
    const isDisabled = filteredOptions.length === 0 ||
        !Boolean(filteredOptions.find(key => selectionState.selectionManager.isSelected(key)));
    return (<button type="button" className={classnames(styles.button, isDisabled && styles.isDisabled)} aria-disabled={isDisabled} onClick={(): void => {
            !isDisabled &&
                selectionState.selectionManager.setSelectedKeys(selectedOptions.filter(option => !filteredOptions.includes(option)));
        }
