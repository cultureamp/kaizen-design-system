...
export const SelectAllButton = (): JSX.Element => {
    ...
    const disabledOptions = selectionState.disabledKeys
        ...
        : [];
    const filteredOptions = Array.from(selectionState.collection.getKeys()).filter(key => !disabledOptions.includes(key));
    return (<button type="button" className={classnames(styles.button, selectionState.selectionManager.isSelectAll && styles.isDisabled)} aria-disabled={selectionState.selectionManager.isSelectAll} onClick={(): false | void => !selectionState.selectionManager.isSelectAll &&
