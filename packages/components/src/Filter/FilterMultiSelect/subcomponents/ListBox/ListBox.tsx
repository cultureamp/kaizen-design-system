...
const getItemsFromKeys = (items: Collection<MultiSelectItem>, keys: Set<Key>): MultiSelectItem[] => {
...
export const ListBox = ({ children }: ListBoxProps): JSX.Element => {
    ...
    const { collection: items, disabledKeys, selectionManager: { selectedKeys }, } = selectionState;
    const disabledItems = getItemsFromKeys(items, disabledKeys);
    const selectedItems = getItemsFromKeys(items, selectedKeys);
    const unselectedItems = Array.from(items).filter(item => !disabledKeys.has(item.key) && !selectedKeys.has(item.key));
    ...
    return (<ul {...listBoxProps} ref={listRef} className={classnames(styles.listBox, isOverflown && styles.overflown)}>
      {children(itemsState)}
    </ul>);
};
