...
export const FilterTriggerButton = ({ selectedOptionLabels, label, classNameOverride, labelCharacterLimitBeforeTruncate = 50, }: FilterTriggerButtonProps): JSX.Element => {
    const { buttonProps, buttonRef, menuTriggerState } = useMenuTriggerContext();
    const ref = useRef<FilterTriggerRef>({ triggerRef: buttonRef });
    return (<FilterButton ref={ref} {...buttonProps} label={label} selectedValue={getTruncatedLabels(selectedOptionLabels, labelCharacterLimitBeforeTruncate)} classNameOverride={classNameOverride} isOpen={menuTriggerState.isOpen}/>);
};
