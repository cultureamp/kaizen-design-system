...
const withQueryProvider: Decorator = Story => (<QueryClientProvider client={client}>
    <Story />
  </QueryClientProvider>);
const meta = {
    ...
    args: {
        ...
        trigger: (): JSX.Element => (<FilterMultiSelect.TriggerButton selectedOptionLabels={["Front-End"]} label="Engineer"/>),
        children: (): JSX.Element => (<>
        <FilterMultiSelect.SearchInput />
        <FilterMultiSelect.ListBox>
          {({ allItems, hasNoItems }): JSX.Element | JSX.Element[] => {
                if (hasNoItems) {
                    return (<FilterMultiSelect.NoResults>
                  No results found.
...
const FilterMultiSelectTemplate: Story = {
    render: args => {
        ...
        return (<FilterMultiSelect {...args} onSelectionChange={setSelectedKeys} selectedKeys={selectedKeys} trigger={(): JSX.Element => (<FilterMultiSelect.TriggerButton selectedOptionLabels={getSelectedOptionLabels(selectedKeys, args.items)} label={args.label}/>)}/>);
    },
};
...
export const Async: Story = {
    render: () => {
        ...
        const fetchSWAPI = async ({ pageParam, }: {
            pageParam: string;
        }): Promise<{
            results: Array<{
                name: string;
        ...
        const currentPeople = React.useMemo(() => data?.pages
            .flatMap(res => res.results)
            .flatMap(person => ({ label: person.name, value: person.url })) || [], [data]);
        /**
        ...
        <FilterMultiSelect.ListBox>
          {({ selectedItems, unselectedItems, disabledItems, hasNoItems, }): JSX.Element => (<>
              ...
              {disabledItems.length > 0 &&
                    (selectedItems.length > 0 || unselectedItems.length > 0) && (<FilterMultiSelect.SectionDivider />)}
...
export const WithSectionHeaders: Story = {
    ...
    args: {
        ...
        <FilterMultiSelect.SearchInput />
        <FilterMultiSelect.ListBox>
          {({ selectedItems, unselectedItems, disabledItems, hasNoItems, }): JSX.Element => hasNoItems ? (<FilterMultiSelect.NoResults>
                No results found.
              </FilterMultiSelect.NoResults>) : (<>
