...
const SelectionContext = React.createContext<SelectionProviderContextType>({} as SelectionProviderContextType);
export const SelectionProvider = (props: SelectionProviderProps): JSX.Element => {
    ...
    const searchFilter = useCallback((nodes: Iterable<MultiSelectItem>): Iterable<MultiSelectItem> => searchQuery
        ? Array.from(nodes).filter(f => f.textValue.toLowerCase().includes(searchQuery.toLowerCase()))
        : nodes, [searchQuery]);
...
export const SelectionConsumer = ({ children, }: React.ConsumerProps<SelectionProviderContextType>): JSX.Element => (<SelectionContext.Consumer>{children}</SelectionContext.Consumer>);
