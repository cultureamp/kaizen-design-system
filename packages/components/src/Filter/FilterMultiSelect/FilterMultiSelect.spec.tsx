...
const FilterMultiSelectWrapper = ({ selectedKeys: propsSelectedKeys, ...props }: Partial<FilterMultiSelectProps>): JSX.Element => {
    const [selectedKeys, setSelectedKeys] = useState<Selection>(propsSelectedKeys ?? new Set(["id-fe"]));
    const handleSelectionChange = (keys: Selection): void => setSelectedKeys(keys);
    return (<FilterMultiSelect label="Engineer" onSelectionChange={handleSelectionChange} selectedKeys={selectedKeys} items={mockItems} trigger={(): JSX.Element => (<FilterMultiSelect.TriggerButton selectedOptionLabels={getSelectedOptionLabels(selectedKeys, mockItems)} label="Engineer"/>)} {...props}>
      {(): JSX.Element => (<>
...
describe("<FilterMultiSelect>", () => {
    ...
    it("filters out options which do not match the search term", async () => {
        const { getByRole, getAllByRole } = renderWithIntl(<FilterMultiSelectWrapper isOpen/>);
    ...
    describe("Clear button", () => {
        it("deselects all options when unfiltered", async () => {
            const { getByRole, queryAllByRole } = renderWithIntl(<FilterMultiSelectWrapper isOpen selectedKeys={new Set(["id-fe", "id-devops"])}/>);
    ...
    describe("Select all button", () => {
        const getDisabledOptions = (options: HTMLElement[]): HTMLElement[] => options.filter(option => option.hasAttribute("aria-disabled"));
        it("selects all non-disabled options when unfiltered", async () => {
            const { getByRole, queryAllByRole } = renderWithIntl(<FilterMultiSelectWrapper isOpen selectedKeys={new Set([])}/>);
