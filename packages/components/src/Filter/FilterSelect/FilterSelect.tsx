...
import { ListBoxSection, ListItem, Option, SectionDivider, SelectPopoverContents, SelectPopoverContentsProps, } from "~components/__future__/Select/subcomponents";
...
export const FilterSelect = <Option extends SelectOption = SelectOption>({ isOpen, setIsOpen, renderTrigger, label, children, items, classNameOverride, selectedKey, ...restProps }: FilterSelectProps<Option>): JSX.Element => {
    // Ref will be populated by Filter
    const [triggerRef, setTriggerRef] = useState<React.RefObject<HTMLButtonElement>>({ current: null });
    ...
    return (<>
      ...
      <Filter isOpen={isOpen} setIsOpen={setIsOpen} renderTrigger={(): JSX.Element => renderTrigger({
            selectedValue: state.selectedItem?.textValue || undefined,
            label,
            isOpen,
            ...buttonProps,
        })} onMount={setTriggerRef} classNameOverride={classNameOverride}>
