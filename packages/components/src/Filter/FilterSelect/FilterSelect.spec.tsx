...
const FilterSelectWrapper = ({ isOpen: propsIsOpen = false, items = mixedMockItemsUnordered, selectedKey, onSelectionChange, ...restProps }: Partial<FilterSelectProps>): JSX.Element => {
    const [isOpen, setIsOpen] = useState<boolean>(propsIsOpen);
    const [selected, setSelected] = React.useState<FilterSelectProps["selectedKey"]>(selectedKey ?? null);
    return (<FilterSelect label="Coffee" isOpen={isOpen} setIsOpen={setIsOpen} renderTrigger={(triggerButtonProps): JSX.Element => (<FilterButton {...triggerButtonProps}/>)} items={items} selectedKey={selected} onSelectionChange={(key): void => {
            setSelected(key);
...
describe("<FilterSelect>", () => {
    ...
    describe("Trigger", () => {
        ...
        describe("Trigger - Keyboard interaction", () => {
            ...
            it("moves the focus to the first focusable element inside the menu initially", async () => {
                ...
                expect(screen.queryByRole("listbox")).toBeVisible();
                await waitFor(() => {
                    expect(screen.queryByRole("option", { name: "Regular" })).toHaveFocus();
                });
            });
    ...
    describe("Selection", () => {
        it("changes the value within the button when an option is selected", async () => {
            render(<FilterSelectWrapper isOpen renderTrigger={(triggerButtonProps): JSX.Element => (<FilterButton data-testid="test__trigger" {...triggerButtonProps}/>)}/>);
    ...
    describe("Number values", () => {
        it("finds selected option when value is a number", () => {
            const { getByRole } = render(<FilterSelectWrapper items={[
                    { value: 0, label: "Zero" },
                    { value: 50, label: "50" },
                    { value: 100, label: "100" },
                ]} selectedKey={50}/>);
...
describe("FilterSelect generic", () => {
    ...
    describe("Extended option", () => {
        ...
        it("should allow consumer to access custom attributes in children", () => {
            ...
            <FilterSelect<SelectOption & {
                isRubberDuck: boolean;
            }> {...defaultProps} items={[{ label: "Bubblegum", value: "bubblegum", isRubberDuck: true }]}>
        {({ items }): JSX.Element[] => items.map(item => item.type === "item" ? (<li key={item.key}>{item.value?.isRubberDuck}</li>) : (<li key={item.key}>Section</li>))}
      </FilterSelect>;
        });
    });
});
