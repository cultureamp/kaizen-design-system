...
const filtersRemovable = [
    {
        id: "flavour",
        name: "Flavour",
        Component: (<FilterBar.Select items={[{ value: "jasmine-milk-tea", label: "Jasmine Milk Tea" }]}/>),
        isRemovable: true,
    },
    {
        id: "topping",
...
const FilterBarWrapper = <T extends FiltersValues>({ filters, defaultValues, ...customProps }: Partial<FilterBarProps<T>> & {
    ...
    return (<>
      <FilterBar<T> filters={filters} values={activeValues} onValuesChange={setActiveValues} {...customProps}/>
...
describe("<FilterBar />", () => {
    it("renders filters in the provided order", async () => {
        const { getAllByTestId } = renderWithIntl(<FilterBarWrapper<ValuesSimple> filters={simpleFilters}/>);
    ...
    describe("Removable filters", () => {
        it("shows inactive filters in the Add Filters menu only", async () => {
            const { getByRole, queryByText } = renderWithIntl(<FilterBarWrapper<ValuesRemovable> filters={filtersRemovable}/>);
            await waitForI18nContent();
        ...
        it("shows removable filter as active if there is a default value", async () => {
            const { getByRole } = renderWithIntl(<FilterBarWrapper<ValuesRemovable> filters={filtersRemovable} defaultValues={{ topping: "pearls" }}/>);
            await waitForI18nContent();
        ...
        it("adds new filters in the provided order", async () => {
            const { getByRole, getAllByTestId } = renderWithIntl(<FilterBarWrapper<ValuesSimple> filters={simpleFilters.map(filter => ({
                    ...filter,
                    isRemovable: true,
                }))}/>);
            await waitForI18nContent();
    ...
    describe("Dependent filters", () => {
        describe("Condition not met", () => {
            it("does not show a dependent filter", async () => {
                const { queryByRole, getByRole } = renderWithIntl(<FilterBarWrapper filters={filtersDependent}/>);
                await waitForI18nContent();
            ...
            it("clears the value if the filter is not usable", async () => {
                const { getByTestId } = renderWithIntl(<FilterBarWrapper filters={filtersDependent} defaultValues={{
                        topping: "pearls",
                    }}/>);
                await waitForI18nContent();
                expect(getByTestId("testid__values").textContent).toEqual(JSON.stringify({}));
            });
        });
        describe("Condition met", () => {
            it("shows a non-removable dependent filter in active filters", async () => {
                const { queryByRole, getByRole, findByRole } = renderWithIntl(<FilterBarWrapper filters={filtersDependent}/>);
                await waitForI18nContent();
            ...
            it("shows a removable dependent filter in Add Filters menu", async () => {
                const { getByRole, findByRole } = renderWithIntl(<FilterBarWrapper filters={[
                        {
                            id: "flavour",
                            name: "Flavour",
                            Component: (<FilterBar.Select items={[
        ...
        describe("Condition result change", () => {
            it("clears the value for an unusable filter", async () => {
                const checkValues = jest.fn<void, [
                    Partial<ValuesDependent>
                ]>();
        ...
        describe("Multiple dependencies", () => {
            it("handles complex dependencies", async () => {
                ...
                const { queryByRole, getByRole } = renderWithIntl(<FilterBarWrapper filters={filters} defaultValues={{ milk: "full-cream" }}/>);
                await waitForI18nContent();
    ...
    describe("Clear all", () => {
        it("clears all the values of all the filters", async () => {
            const { getByRole } = renderWithIntl(<FilterBarWrapper<ValuesSimple> filters={simpleFilters} defaultValues={{
                    flavour: "jasmine-milk-tea",
                    sugarLevel: 50,
                    iceLevel: 100,
                }}/>);
        ...
        it("removes all removable filters", async () => {
            const { getByRole } = renderWithIntl(<FilterBarWrapper<ValuesRemovable> filters={filtersRemovable} defaultValues={{
                    flavour: "jasmine-milk-tea",
                    topping: "pearls",
                }}/>);
            await waitForI18nContent();
    ...
    describe("Context use cases", () => {
        describe("getActiveFilterValues()", () => {
            ...
            const MockFilterAsyncComponent = ({ id, fetcher, }: {
                id: string;
                ...
                return (<FilterBar.MultiSelect id={id} items={items}>
            {() => (<FilterMultiSelect.ListBox>
                {({ allItems }) => (<FilterMultiSelect.ListBoxSection items={allItems} sectionName="All Items">
            ...
            it("can re-fetch options with all active filter values pulled off of the FilterBarContext", async () => {
                const { getByRole, queryByRole } = renderWithIntl(<FilterBarWrapper<AsyncValues> filters={config} defaultValues={{}}/>);
        ...
        describe("openFilter()", () => {
            ...
            const CycleFilter = ({ id }: {
                ...
                return (<FilterBar.Select id={id} items={[{ value: "custom", label: "Custom Date" }]} onSelectionChange={key => {
                        if (key === "custom")
                            openFilter("customDate");
                    }}/>);
            };
            const cycleFilters = [
                {
            ...
            it("opens the Custom Date filter when Cycle's 'custom' value is selected", async () => {
                const { getByRole } = renderWithIntl(<FilterBarWrapper<CycleFilterValues> filters={cycleFilters}/>);
                await waitForI18nContent();
