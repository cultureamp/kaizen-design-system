...
import { encodeQueryParams, StringParam, ArrayParam, DateParam, encodeDate, encodeObject, decodeDate, decodeObject, QueryParamConfig, decodeQueryParams, } from "serialize-query-params";
...
const filters = [
    ...
    },
    {
        id: "toppings",
        name: "Toppings",
        Component: (<FilterBar.MultiSelect items={[
                { value: "none", label: "None" },
                { value: "pearls", label: "Pearls" },
                { value: "fruit-jelly", label: "Fruit Jelly" },
            ]}>
...
const ExampleFilterMultiSelect = (props: Omit<FilterBarMultiSelectProps, "children">): JSX.Element => (<FilterBar.MultiSelect {...props}>
    {(): JSX.Element => (<FilterMultiSelect.ListBox>
        {({ allItems }): JSX.Element | JSX.Element[] => {
...
const FilterPerson = (props: {
    ...
    const { getFilterState } = useFilterBarContext<ValuesSiblingDependent["person"], ValuesSiblingDependent>();
    ...
    return (<ExampleFilterMultiSelect id={props.id} isLoading={isLoading} loadingSkeleton={<FilterMultiSelect.MenuLoadingSkeleton />} items={items}/>);
};
const FilterRoom = (props: {
    ...
    const { getFilterState } = useFilterBarContext<ValuesSiblingDependent[Id], ValuesSiblingDependent>();
    const roleFilter = getFilterState("role");
    useEffect(() => {
