...
const FilterBarDatePickerWrapper = ({ defaultValues, filterAttributes, ...customProps }: {
    ...
    return (<FilterBarProvider<Values> filters={[
            {
                id: "drank",
                name: "Drank",
                Component: (<FilterBarDatePicker id="drank" defaultMonth={new Date("2023-06-06")} {...customProps}/>),
                ...filterAttributes,
            },
        ]} values={values} onValuesChange={setValues}>
      {filters => (<>
          {Object.values(filters).map(({ id, Component }) => (<React.Fragment key={id}>{Component}</React.Fragment>))}
        </>)}
    </FilterBarProvider>);
};
describe("<FilterBarDatePicker />", () => {
    ...
    describe("Removable", () => {
        it("does not show the remove button when isRemovable is false", async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.queryByRole("button", { name: "Remove filter - Drank" })).not.toBeInTheDocument();
            });
        });
        it("shows the remove button when isRemovable is true", async () => {
            renderWithIntl(<FilterBarDatePickerWrapper filterAttributes={{ isRemovable: true }} defaultValues={{ drank: new Date("2023-05-01") }}/>);
            await waitFor(() => {
    ...
    it("can toggle its open state", async () => {
        const { getByRole, queryByRole } = renderWithIntl(<FilterBarDatePickerWrapper />);
    ...
    it("shows a selected value when provided", async () => {
        const { getByRole } = renderWithIntl(<FilterBarDatePickerWrapper defaultValues={{ drank: new Date("2023-06-06") }}/>);
        await waitFor(() => {
    ...
    it("updates the selected value in the trigger button when selecting a date", async () => {
        renderWithIntl(<FilterBarDatePickerWrapper defaultValues={{ drank: new Date("2023-06-06") }}/>);
