...
const FilterBarButtonWrapper = (props: Partial<FilterBarButtonProps>): JSX.Element => (<FilterBarProvider<Values> filters={[
        {
            id: "coffee",
            name: "Coffee",
            Component: <div />,
        },
    ]} values={{}} onValuesChange={() => undefined}>
    {filters => (<>
        {Object.values(filters).map(({ id, name }) => (<FilterBarButton key={id} filterId={id} label={name} isRemovable={false} {...props}/>))}
      </>)}
  </FilterBarProvider>);
describe("<FilterBarButton />", () => {
    it("does not show a remove button when it is not removable", async () => {
        const { getByRole, queryByRole } = renderWithIntl(<FilterBarButtonWrapper />);
        await waitFor(() => {
