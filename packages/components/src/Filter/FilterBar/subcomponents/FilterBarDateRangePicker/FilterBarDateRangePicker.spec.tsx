...
const FilterBarDateRangePickerWrapper = ({ defaultValues, filterAttributes, ...customProps }: {
...
describe("<FilterBarDateRangePicker />", () => {
    ...
    describe("Removable", () => {
        it("does not show the remove button when isRemovable is false", async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.queryByRole("button", { name: "Remove filter - Dates" })).not.toBeInTheDocument();
            });
        });
        it("shows the remove button when isRemovable is true", async () => {
            const { getByRole } = renderWithIntl(<FilterBarDateRangePickerWrapper filterAttributes={{ isRemovable: true }} defaultValues={{ range: { from: new Date("2023-05-01") } }}/>);
            await waitFor(() => {
    ...
    it("can toggle its open state", async () => {
        const { getByRole, queryByRole } = renderWithIntl(<FilterBarDateRangePickerWrapper />);
    ...
    it("shows a selected value when provided", async () => {
        const { getByRole } = renderWithIntl(<FilterBarDateRangePickerWrapper defaultValues={{
                range: {
    ...
    it("updates the selected value in the trigger button", async () => {
        const { getByRole, getByText } = renderWithIntl(<FilterBarDateRangePickerWrapper defaultValues={{
                range: {
                    from: new Date("2022-05-01"),
