...
import { FilterAttributes, FilterBarProvider, Filters, FiltersValues, useFilterBarContext, } from "~components/Filter/FilterBar";
...
const SelectContents = (): JSX.Element => (<>
    <FilterMultiSelect.ListBox>
      {({ allItems }): JSX.Element | JSX.Element[] => allItems.map(item => (<FilterMultiSelect.Option key={item.key} item={item}/>))}
    </FilterMultiSelect.ListBox>
...
const FilterBarMultiSelectWrapper = <ValuesMap extends FiltersValues = Values>({ defaultValues, filterAttributes, additionalFilters = [], ...customProps }: {
    ...
    return (<FilterBarProvider<ValuesMap> filters={[
            {
                id: "toppings",
                name: "Toppings",
                Component: (<FilterBarMultiSelect id="toppings" items={[
      ...
      {filters => (<>
          {Object.values(filters).map(({ id, Component }) => (<React.Fragment key={id as string}>{Component}</React.Fragment>))}
...
describe("<FilterBarMultiSelect />", () => {
    ...
    describe("Removable", () => {
        ...
        it("shows the remove button when isRemovable is true", async () => {
            const { getByRole } = renderWithIntl(<FilterBarMultiSelectWrapper filterAttributes={{ isRemovable: true }} defaultValues={{ toppings: ["pearls"] }}/>);
            await waitFor(() => {
    ...
    it("can toggle its open state", async () => {
        const { getByRole, queryByRole } = renderWithIntl(<FilterBarMultiSelectWrapper />);
        const triggerButton = getByRole("button", { name: "Toppings" });
    ...
    it("shows a selected value when provided", async () => {
        const { getByRole } = renderWithIntl(<FilterBarMultiSelectWrapper defaultValues={{ toppings: ["pearls", "fruit-jelly"] }}/>);
        await waitFor(() => {
    ...
    it("updates the selected value in the trigger button", async () => {
        const { getByRole } = renderWithIntl(<FilterBarMultiSelectWrapper defaultValues={{ toppings: ["pearls"] }}/>);
        const triggerButton = getByRole("button", {
            name: "Toppings : Pearls",
        });
        await user.click(triggerButton);
    ...
    it("allows calling additional functions on selection change", async () => {
        ...
        const { getByRole } = renderWithIntl(<FilterBarMultiSelectWrapper onSelectionChange={onChange}/>);
        const triggerButton = getByRole("button", { name: "Toppings" });
        await user.click(triggerButton);
        await waitFor(() => {
            const listbox = getByRole("listbox");
    ...
    it("clears selected values if updated items do not contain the value", async () => {
        ...
        const FilterFlavour = (): JSX.Element => {
            ...
            return (<FilterBarMultiSelect id="flavour" items={allItems}>
          {(): JSX.Element => <SelectContents />}
        </FilterBarMultiSelect>);
        };
        const { getByRole, getAllByRole, getByTestId } = renderWithIntl(<FilterBarMultiSelectWrapper<ValuesDependent> additionalFilters={[
                {
                    id: "flavour",
                    name: "Flavour",
        ...
        await waitFor(() => {
            expect(getByTestId("json-values").textContent).toBe(JSON.stringify({
