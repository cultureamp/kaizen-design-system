...
import { FilterMultiSelect, FilterMultiSelectProps, ItemType, getSelectedOptionLabels, } from "~components/Filter/FilterMultiSelect";
...
const convertConsumableFormatIntoSelection = (value: ConsumableSelection): Selection => new Set(value);
export const FilterBarMultiSelect = ({ id, items: propsItems, children, onSelectionChange, ...props }: FilterBarMultiSelectProps): JSX.Element | null => {
    const { getFilterState, setFilterOpenState, updateValue, hideFilter } = useFilterBarContext<ConsumableSelection>();
    ...
    return (<FilterMultiSelect label={filterState.name} 
    // Convert the incoming FilterBar state to a Set (internal FilterMultiSelect state)
    selectedKeys={new Set(filterState.value || null)} onSelectionChange={(keys): void => {
        ...
        }} items={items} isOpen={filterState.isOpen} onOpenChange={(open): void => setFilterOpenState(id, open)} trigger={(): JSX.Element => {
            const triggerProps = {
                selectedOptionLabels: filterState.value
                    ? getSelectedOptionLabels(convertConsumableFormatIntoSelection(filterState.value), items)
                    : [],
                label: filterState.name,
            };
            return filterState.isRemovable ? (<FilterMultiSelect.RemovableTrigger {...triggerProps} onRemove={() => hideFilter(id)}/>) : (<FilterMultiSelect.TriggerButton {...triggerProps}/>);
        }} {...props}>
      {children}
    </FilterMultiSelect>);
};
