...
const FilterBarSelectWrapper = <ValuesMap extends FiltersValues = Values>({ defaultValues, filterAttributes, additionalFilters = [], ...customProps }: {
    ...
    return (<FilterBarProvider<ValuesMap> filters={[
            {
                id: "flavour",
                name: "Flavour",
                Component: (<FilterBarSelect id="flavour" items={[
...
describe("<FilterBarSelect />", () => {
    ...
    describe("Removable", () => {
        it("does not show the remove button when isRemovable is false", async () => {
            ...
            expect(screen.queryByRole("button", { name: "Remove filter - Flavour" })).not.toBeInTheDocument();
        });
        it("shows the remove button when isRemovable is true", async () => {
            renderWithIntl(<FilterBarSelectWrapper filterAttributes={{ isRemovable: true }} defaultValues={{ flavour: "jasmine-milk-tea" }}/>);
            await waitFor(() => {
    ...
    it("can toggle its open state", async () => {
        const { getByRole, queryByRole } = renderWithIntl(<FilterBarSelectWrapper />);
    ...
    it("shows a selected value when provided", async () => {
        const { getByRole } = renderWithIntl(<FilterBarSelectWrapper defaultValues={{ flavour: "jasmine-milk-tea" }}/>);
        await waitFor(() => {
    ...
    it("allows calling additional functions on selection change", async () => {
        ...
        const { getByRole } = renderWithIntl(<FilterBarSelectWrapper onSelectionChange={onChange}/>);
        const triggerButton = getByRole("button", { name: "Flavour" });
        await user.click(triggerButton);
        await waitFor(() => {
            const listbox = getByRole("listbox");
    ...
    it("clears selected values if updated items do not contain the value", async () => {
        ...
        const { getByRole, getAllByRole, getByTestId } = renderWithIntl(<FilterBarSelectWrapper<ValuesDependent> additionalFilters={[
                {
                    id: "topping",
                    name: "Topping",
