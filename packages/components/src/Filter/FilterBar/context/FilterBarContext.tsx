...
export const useFilterBarContext = <Value, Values extends FiltersValues = Record<string, Value>>(): FilterBarContextValue<Value, Values> => {
...
export const FilterBarProvider = <ValuesMap extends FiltersValues>({ children, filters, values, onValuesChange, }: FilterBarProviderProps<ValuesMap>): JSX.Element => {
    const mappedFilters = useMemo(() => getMappedFilters(filters), [filters]);
    const [state, dispatch] = useReducer(filterBarStateReducer<ValuesMap>, setupFilterBarState<ValuesMap>(filters, values));
    const value = {
        ...
        showFilter: <Id extends keyof ValuesMap>(id: Id): void => dispatch({ type: "activate_filter", id }),
        hideFilter: <Id extends keyof ValuesMap>(id: Id): void => {
