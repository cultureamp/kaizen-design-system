...
describe("<Collapsible />", () => {
    it("includes the 'sticky' class on buttons when the 'sticky' prop is specified", () => {
        const { getByTestId } = render(<Collapsible id="1" title="First panel" sticky={{ top: "0px" }}>
        First panel content
      </Collapsible>);
    ...
    it("toggles the height of the section on click of the button", async () => {
        const { getByTestId } = render(<Collapsible id="1" open title="First panel">
        First panel content
      </Collapsible>);
    ...
    it("gives precedence to renderHeader over title", () => {
        const { container, getByTestId } = render(<Collapsible id="1" open title="Should not be rendered" renderHeader={(): JSX.Element => (<div>This title should be rendered</div>)}>
        First panel content
      </Collapsible>);
    ...
    it("doesn't render section content when lazyLoad is enabled", () => {
        const { container } = render(<Collapsible id="1" title="Title" lazyLoad>
    ...
    it("runs the onToggle callback", async () => {
        ...
        const { getByTestId } = render(<Collapsible id="1" open title="First panel" onToggle={onToggle}>
        First panel content
      </Collapsible>);
        const collapsible = getByTestId("collapsible-header-1");
        await user.click(collapsible);
    ...
    it("respects controlled mode (stays open after click)", async () => {
        const { getByTestId } = render(<Collapsible id="1" open title="First panel" controlled>
        First panel content
      </Collapsible>);
        const collapsible = getByTestId("collapsible-header-1");
        const section = getByTestId("collapsible-section-1");
