...
export const ToastNotificationsMap = ({ notifications, onHide: defaultOnHide, container, }: ToastNotificationsMapProps): JSX.Element | null => {
    if (!container)
        return null;
    ...
    return createPortal(<>
      {notifications.map(({ id, type, title, persistent, onHide, message, ...restProps }) => (<GenericNotification key={id} style="toast" type={type} title={title} persistent={persistent} onHide={(): void => {
