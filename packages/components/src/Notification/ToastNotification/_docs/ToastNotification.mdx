...
The use of the `persistent` or `hideCloseIcon` props should be used with caution as they will need to be paired with a dismissable trigger or a means of removing the messages. You can leverage the `clearToastNotifications` function to add a clear all trigger.
