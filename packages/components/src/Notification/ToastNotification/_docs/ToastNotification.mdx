...
<Installation
  ...
  importStatement='import { addToastNotification, updateToastNotification, removeToastNotification, clearToastNotifications, ToastNotification } from "@kaizen/components"'
/>
...
In addition to the `ToastNotification` JSX element, there are helper methods available through
the `useToastNotification` hook to create and manage Toast notifications.
