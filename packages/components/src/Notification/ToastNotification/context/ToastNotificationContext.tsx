...
export const ToastNotificationProvider = ({ children, }: ToastNotificationProviderProps): JSX.Element | null => {
    const [notifications, setNotifications] = useState<ToastNotificationObj[]>([]);
    const addToastNotification: ToastNotificationContextValue["addToastNotification"] = notification => {
        const uuid = uuidv4();
        const notificationWithId = { id: uuid, ...notification };
        const notificationExists = notifications.find(({ id }) => id === notification.id);
    ...
    return (<ToastNotificationContext.Provider value={value}>
      {children}
