...
export const GenericNotification = ({ type, style, children, title, persistent = false, onHide, noBottomMargin, forceMultiline, headingProps, classNameOverride, ...restProps }: GenericNotificationProps): JSX.Element | null => {
    ...
    return (<div ref={containerRef} className={classnames(styles.notification, styles[type], styles[style], isHidden && styles.hidden, noBottomMargin && styles.noBottomMargin, classNameOverride, persistent && styles.persistent)} style={{ marginTop: getMarginTop() }} onTransitionEnd={onTransitionEnd} {...restProps}>
      <div className={styles.icon}>
        <NotificationIcon type={type}/>
      </div>
      <div className={classnames(styles.textContainer, forceMultiline && styles.forceMultiline)}>
        {style !== "global" && (<NotificationHeading titleProp={title} headingProps={headingProps}/>)}
