...
const renderCounter = (remainingAvatars: number): JSX.Element | void => {
    if (remainingAvatars <= 0)
        return;
    return (<li aria-label={`There ${remainingAvatars > 1
            ? `are ${remainingAvatars} other members`
            : `is ${remainingAvatars} other member`} of this group`} className={styles.AvatarGroupItem}>
      <span className={styles.AvatarCounter} aria-hidden={true}>
        {`+${remainingAvatars}`}
      </span>
    </li>);
};
const renderAvatars = (avatars: AvatarList, maxVisible: number, size: AvatarGroupSize): JSX.Element => (<>
    {avatars?.map((avatarProps, index) => index < maxVisible && (<li key={`avatar-${index}`} className={styles.AvatarGroupItem}>
 ...
 */
export const AvatarGroup = ({ size = "medium", maxVisible = 2, avatars, classNameOverride, ...restProps }: AvatarGroupProps): JSX.Element => (<ul className={classnames(styles.AvatarGroup, styles[size], classNameOverride)} aria-label="Avatar Group" {...restProps}>
