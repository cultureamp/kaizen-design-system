...
export const MenuDropdown = ({ children, referenceElement, id, hideMenuDropdown, autoHide = "on", align = "left", width = "default", }: MenuDropdownProps): JSX.Element => {
    ...
    const handleDocumentClickForAutoHide = useCallback((e: MouseEvent) => {
        if (popperElement &&
    ...
    return (<FocusOn enabled={!!referenceElement} scrollLock={false} noIsolation shards={referenceElement ? [referenceElement] : undefined} onEscapeKey={hideMenuDropdown}>
      {/* eslint-disable-next-line
                jsx-a11y/click-events-have-key-events,
                jsx-a11y/no-static-element-interactions */}
      <div id={id} ref={setPopperElement} {...attributes.popper} style={popperStyles.popper} className={classnames(styles.menuContainer, width == "default" && styles.defaultWidth)} onClick={handleRootClick}>
        {children}
      </div>
    </FocusOn>);
};
