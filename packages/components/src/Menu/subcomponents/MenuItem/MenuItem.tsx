...
export const MenuItem = ({ label, icon, destructive, disabled, onClick, href, target, isActive, "data-testid": dataTestId, id, }: MenuItemProps): JSX.Element => {
    ...
    const className = classnames(styles.menuItem, destructive && styles["menuItem--destructive"], disabled && styles["menuItem--disabled"], isActive && styles["menuItem--active"]);
    if (disabled) {
        return (<li className={styles.menuListItem}>
        <button id={id} type="button" disabled={true} className={className} data-testid={dataTestId}>
          {iconNode}
          {wrappedLabel}
        </button>
      </li>);
    ...
    if (href) {
        return (<li className={styles.menuListItem}>
        <a id={id} href={href} className={className} target={target} 
        ...
        aria-current={isActive ? "page" : undefined} data-testid={dataTestId}>
          {iconNode}
          {wrappedLabel}
        </a>
      </li>);
    ...
    return (<li className={styles.menuListItem}>
      <button id={id} type="button" onClick={onClick} className={className} data-testid={dataTestId}>
        {iconNode}
        {wrappedLabel}
      </button>
    </li>);
