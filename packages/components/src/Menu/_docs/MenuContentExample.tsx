...
export const MenuContentExample = ({ isShortList, }: MenuContentExampleProps): JSX.Element => (<>
    ...
    {!isShortList && (<>
        <MenuList heading={<MenuHeading>Buttons</MenuHeading>}>
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            alert("Hello");
            e.preventDefault();
        }} icon={<EditIcon role="presentation"/>} label="Menu button"/>
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            e.preventDefault();
        }} icon={<DuplicateIcon role="presentation"/>} label="Menu button but the label is too long"/>
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            e.preventDefault();
        }} icon={<TrashIcon role="presentation"/>} destructive label="Destructive Menu button"/>
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            e.preventDefault();
        }} icon={<TrashIcon role="presentation"/>} disabled label="Disabled Menu button"/>
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            e.preventDefault();
        }} icon={<TrashIcon role="presentation"/>} disabled destructive label="Disabled Destructive Menu button"/>
        </MenuList>
        <MenuList heading={<MenuHeading>Buttons (no icons)</MenuHeading>}>
          ...
          <MenuItem onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
            e.preventDefault();
        }} disabled label="Disabled button no icon"/>
        </MenuList>
      </>)}
  </>);
