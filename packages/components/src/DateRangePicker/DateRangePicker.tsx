 ...
 */
export const DateRangePicker = ({ id: propsId, buttonRef = useRef<HTMLButtonElement>(null), description: _description, // not used
labelText, isDisabled = false, classNameOverride, disabledDates, disabledDaysOfWeek, disabledRange, disabledBeforeAfter, disabledBefore, disabledAfter, weekStartsOn, defaultMonth, selectedDateRange, value, onChange, ...inputProps }: DateRangePickerProps): JSX.Element => {
    const reactId = useId();
    ...
    const isSelectingFirstDay = (range: DateRange, day: Date): boolean => {
        const isBeforeFirstDay = !!range.from &&
            isMatch(day, [
                {
                    before: range.from,
                },
    ...
    return (<div>
      <div ref={containerRef} className={classNameOverride}>
        ...
        <button type="button" id={id} className={cx(styles.button, styles.withStartIconAdornment, {
            [styles.disabled]: isDisabled,
        })} disabled={isDisabled} ref={buttonRef} onClick={handleOpenClose} aria-label={selectedDateRange?.from ? `Change date: ${value}` : "Choose date"} {...inputProps}>
      ...
      {isOpen && (<FocusOn scrollLock={false} onDeactivation={handleReturnFocus} onClickOutside={handleOpenClose} onEscapeKey={handleOpenClose} enabled={isOpen}>
          <CalendarPopover referenceElement={containerRef.current} aria-labelledby={`${id}-calendar-label ${id}-input-label`}>
            <VisuallyHidden id={`${id}-calendar-label`}>
              Select dates from calendar for:
            </VisuallyHidden>
            <LegacyCalendarRange selectedRange={selectedDateRange} defaultMonth={defaultMonth} weekStartsOn={weekStartsOn} disabledDays={disabledDays} onDayChange={handleDayClick} locale={enUS}/>
          </CalendarPopover>
        </FocusOn>)}
    </div>);
};
