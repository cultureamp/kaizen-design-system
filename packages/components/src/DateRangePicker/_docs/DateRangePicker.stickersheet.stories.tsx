...
import { LegacyCalendarRange, LegacyCalendarRangeProps, } from "~components/Calendar/LegacyCalendarRange";
...
const DateRangePickerTemplate = (props: Partial<DateRangePickerProps>): JSX.Element => {
    ...
    return (<>
      <DateRangePicker labelText="Label" onChange={onDateRangeChange} value={value} selectedDateRange={selectedDateRange} {...props}/>
    </>);
};
const LegacyCalendarRangeTemplate = (props: Partial<LegacyCalendarRangeProps>): JSX.Element => {
    ...
    return (<LegacyCalendarRange onDayChange={(): void => undefined} weekStartsOn={0} defaultMonth={new Date(2022, 2)} selectedRange={selectedDateRange} locale={enAU} {...props}/>);
};
const selectedDateRange = {
...
const StickerSheetTemplate: StickerSheetStory = {
    render: () => (<>
      <StickerSheet>
        <StickerSheet.Header headings={["Default", "Selected Value", "Disabled"]} hasVerticalHeadings headingsWidth={250}/>
        <StickerSheet.Row rowTitle="Date Range Picker Input">
          <DateRangePickerTemplate />
          <DateRangePickerTemplate selectedDateRange={selectedDateRange} value="Mar 6 – Mar 2, 2022"/>
          <DateRangePickerTemplate isDisabled/>
        </StickerSheet.Row>
      </StickerSheet>

      <StickerSheet>
        <StickerSheet.Header headings={["Selected Range Dates", "Disabled Dates"]} hasVerticalHeadings/>
        <StickerSheet.Row rowTitle="Date Range Calendar (Legacy)">
          <LegacyCalendarRangeTemplate selectedRange={selectedDateRange}/>
          <LegacyCalendarRangeTemplate disabledDays={[
            new Date(2022, 1, 15),
