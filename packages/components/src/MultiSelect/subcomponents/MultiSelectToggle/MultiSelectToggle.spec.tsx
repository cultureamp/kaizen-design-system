...
const MultiSelectToggleWrapper = (customProps?: Partial<MultiSelectToggleProps>): JSX.Element => (<>
    ...
    <MultiSelectToggle selectedOptions={[]} onClick={onClick} isOpen={false} aria-labelledby="id--label" aria-controls="id--dialog" {...customProps} onRemoveOption={() => undefined} onRemoveAllOptions={() => undefined}/>
...
describe("<MultiSelectToggle />", () => {
    ...
    describe("Has no selected options", () => {
        it("does not show the clear all button", () => {
            ...
            expect(queryByRole("button", { name: "Remove all options from Waffle" })).not.toBeInTheDocument();
        });
    });
    describe("Has selected options", () => {
        it("does not call onClick when clearing a single selected item", async () => {
            const { getByRole } = render(<MultiSelectToggleWrapper selectedOptions={[{ value: "waffle", label: "Waffle" }]}/>);
