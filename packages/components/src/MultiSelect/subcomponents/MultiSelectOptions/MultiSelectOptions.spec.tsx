...
const MultiSelectOptionsWrapper = (customProps?: Partial<MultiSelectOptionsProps>): JSX.Element => {
    const [selectedValues, setSelectedValues] = useState<MultiSelectOptionsProps["selectedValues"]>(customProps?.selectedValues ?? new Set());
    return (<MultiSelectOptions id="id--options" options={testOptions} {...customProps} selectedValues={selectedValues} onChange={newValues => {
...
describe("<MultiSelectOptions />", () => {
    ...
    describe("Has options", () => {
        ...
        it("renders the correct checked status for each option", () => {
            const { getAllByRole } = render(<MultiSelectOptionsWrapper selectedValues={new Set(["waffle"])}/>);
            const options = getAllByRole("checkbox");
        ...
        it("returns updated selected values when selecting an option", async () => {
            const { getByRole, queryAllByRole } = render(<MultiSelectOptionsWrapper selectedValues={new Set(["pancakes"])}/>);
        ...
        it("returns updated selected values when unselecting an option", async () => {
            const { getByRole, queryAllByRole } = render(<MultiSelectOptionsWrapper selectedValues={new Set(["pancakes", "waffle"])}/>);
            expect(queryAllByRole("presentation", { hidden: true }).length).toBe(2);
            const waffleOption = getByRole("checkbox", { name: "Waffle" });
            expect(waffleOption).toBeChecked();
            await user.click(waffleOption);
