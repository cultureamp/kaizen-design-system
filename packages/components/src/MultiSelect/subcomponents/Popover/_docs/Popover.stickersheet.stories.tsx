...
const PopoverTemplate = (args: Partial<Omit<PopoverProps, "refs">>): JSX.Element => {
    ...
    return (<div>
      <button ref={refs.setReference} type="button" className="border border-gray-500" onClick={() => setIsOpen(!isOpen)}>
        Pancakes!
      </button>
      {isOpen && (<Popover refs={refs} aria-label="Pancakes!" {...args}>
...
const PopoverWithPortal = ({ portalClassName, ...props }: Partial<Omit<PopoverProps, "refs">> & {
    ...
    className={classnames("relative border border-purple-500 overflow-hidden", portalClassName)}>
      <PopoverTemplate portalContainer={portalContainer} focusOnProps={{ enabled: false }} {...props}/>
    </div>);
};
const List = ({ items }: {
    items: string[];
