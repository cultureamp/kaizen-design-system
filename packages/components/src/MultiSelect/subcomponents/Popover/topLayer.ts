 ...
 * mounted. No-op when the element is absent or already marked (eg. marked by the consumer).
 *
...
export const useTopLayer = (element: Element | DocumentFragment | undefined): void => {
  useLayoutEffect(() => {
    if (!(element instanceof Element) || element.hasAttribute(TOP_LAYER_ATTRIBUTE)) return
