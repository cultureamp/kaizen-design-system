    ...
    render: () => (<StickerSheet>
      ...
      <StickerSheet.Body>
        {STATUS_ROWS.map(({ title, status }) => (<StickerSheet.Row key={title} rowTitle={title}>
            ...
            <Checkbox aria-label="Hover label" classNameOverride="story__checkbox--hover" checkedStatus={status} readOnly/>
            <Checkbox aria-label="Focus label" classNameOverride="story__checkbox--focus" checkedStatus={status} readOnly/>
          </StickerSheet.Row>))}
      </StickerSheet.Body>
    </StickerSheet>),
    parameters: {
