...
export const MultiSelect = ({ id: propsId, label, items, selectedValues, description, onSelectedValuesChange, isOpen, onOpenChange, classNameOverride, validationMessage, ...restProps }: MultiSelectProps): JSX.Element => {
    ...
    return (<div id={id} className={classnames(classNameOverride)} {...restProps}>
      <Heading tag="span" variant="heading-6" id={`${id}--label`}>
        {label}
      </Heading>

      <div ref={refs.setReference} className={styles.toggleContainer}>
        <MultiSelectToggle ref={toggleButtonRef} id={`${id}--toggle`} aria-labelledby={`${id}--label`} aria-describedby={`${validationId} ${descriptionId}`} aria-controls={`${id}--popover`} onClick={handleToggleClick} isOpen={isOpen} selectedOptions={Array.from(selectedValues).map(value => itemsMap[value])} status={validationMessage?.status} onRemoveOption={handleOnRemoveOption} onRemoveAllOptions={handleRemoveAllOptions}/>
      </div>
      ...
      {isOpen && (<Popover refs={refs} id={`${id}--popover`} focusOnProps={{
                enabled: true,
                onClickOutside,
          ...
          <MultiSelectOptions id={`${id}--options`} options={items} onChange={onSelectedValuesChange} selectedValues={selectedValues}/>
        </Popover>)}
    </div>);
};
