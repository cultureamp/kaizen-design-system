...
Error and cautionary states for the MultiSelect are controlled via the `status` and `validationMessage` props.

<Canvas of={MultiSelectStories.ValidationMessage} />

A validation message will render below the MultiSelect's description and will be read to assistive technologies after the `description` content.
