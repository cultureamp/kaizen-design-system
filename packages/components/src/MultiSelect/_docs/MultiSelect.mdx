...
The `isDisabled` prop will apply `aria-disabled` to the toggle element. While and click functionality will be disabled, this will allow the field to retain focus so it can be announced to screen readers. This is not a read only state, so disabled MultiSelects should not have SelectedItems

<Canvas of={MultiSelectStories.Disabled} />

<Canvas of={MultiSelectStories.DisabledWithTags} />
