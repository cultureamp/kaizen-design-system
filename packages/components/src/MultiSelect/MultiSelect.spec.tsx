...
const MultiSelectWrapper = ({ isOpen: propsIsOpen = false, selectedValues: propsSelectedValues = new Set(), ...otherProps }: Partial<MultiSelectProps>): JSX.Element => {
    const [selectedValues, setSelectedValues] = useState<MultiSelectProps["selectedValues"]>(propsSelectedValues);
    const [isOpen, setIsOpen] = useState<boolean>(propsIsOpen);
    return (<MultiSelect items={defaultOptions} label="Jalapeno" {...otherProps} isOpen={isOpen} onOpenChange={setIsOpen} selectedValues={selectedValues} onSelectedValuesChange={setSelectedValues}/>);
};
...
describe("<MultiSelect />", () => {
    ...
    describe("id", () => {
        it("uses the consumer-provided id", async () => {
            const { getByTestId, getByRole } = render(<MultiSelectWrapper id="waffle" label="Jalapeno" items={defaultOptions} selectedValues={new Set()} data-testid="test-id--waffle" isOpen/>);
            await waitFor(() => {
    ...
    describe("Close popover", () => {
        ...
        it("does not close the popover when clearing a selected option", async () => {
            const { getByRole, getByLabelText } = render(<MultiSelectWrapper selectedValues={new Set(["pancakes"])}/>);
            const toggleButton = getByRole("button", { name: "Jalapeno" });
            await user.click(toggleButton);
            const popover = getByRole("dialog");
            await waitFor(() => {
    ...
    describe("Focus lock", () => {
        describe("When open", () => {
            it("restricts focusable elements to toggle button and popover contents", async () => {
                const { getByRole } = render(<MultiSelectWrapper items={[{ label: "Pancakes", value: "pancakes" }]} isOpen/>);
                const toggleButton = getByRole("button", { name: "Jalapeno" });
            ...
            describe("When closed", () => {
                it("has expected focus order", async () => {
                    const { getByRole, getByLabelText } = render(<MultiSelectWrapper selectedValues={new Set(["waffle"])}/>);
                    const toggleButton = getByRole("button", { name: "Jalapeno" });
                    await user.tab();
                    await waitFor(() => {
                        expect(toggleButton).toHaveFocus();
    ...
    it("shows selected items in the selected order", async () => {
        const { getByRole, getAllByRole } = render(<MultiSelectWrapper id="jalapeno" selectedValues={new Set(["toastie"])} isOpen/>);
...
describe("Removing an option", () => {
    it("removes the option when the clear button is clicked", async () => {
        const { getByRole, getByText, getByLabelText } = render(<MultiSelectWrapper selectedValues={new Set(["waffle"])}/>);
...
describe("Removing all options", () => {
    it("removes all selected options when the 'clear all' button is clicked", async () => {
        const { getByRole, getByText } = render(<MultiSelectWrapper selectedValues={new Set(["pancakes", "waffle"])}/>);
...
describe("Has validation status", () => {
    it("renders a validation message", () => {
        render(<MultiSelectWrapper selectedValues={new Set(["waffle"])} validationMessage={{
                status: "error",
    ...
    it("describes the Toggle", () => {
        render(<MultiSelectWrapper selectedValues={new Set(["waffle"])} label="Breakfast menu" validationMessage={{
                status: "caution",
    ...
    it("announces the validation message before the Toggle's description", () => {
        ...
        render(<MultiSelectWrapper selectedValues={new Set(["waffle"])} label="Breakfast menu" description={description} validationMessage={{
                status: "caution",
                message: "Only four waffles remain.",
            }}/>);
        expect(screen.getByRole("button", {
