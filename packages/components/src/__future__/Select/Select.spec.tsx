...
const SelectWrapper = ({ items = singleMockItems, selectedKey, onSelectionChange, ...props }: Partial<SelectProps>): JSX.Element => {
    const [selected, setSelected] = React.useState<SelectProps["selectedKey"]>(selectedKey ?? null);
    return (<Select label="Mock Label" items={items} description="This is a description" selectedKey={selected} onSelectionChange={(selection): void => {
...
describe("<Select />", () => {
    describe("Trigger", () => {
        ...
        describe("when controlled", () => {
            ...
            it("fires the onOpenChange callback when the trigger is interacted", async () => {
                ...
                const { getByRole } = render(<SelectWrapper selectedKey="batch-brew" defaultOpen onOpenChange={onOpenChange}/>);
        ...
        describe("Trigger - Mouse interaction", () => {
            ...
            describe("Given the menu is opened", () => {
                it("is closed when user clicks on the trigger", async () => {
                    const { getByRole, queryByRole } = render(<SelectWrapper selectedKey="batch-brew" defaultOpen/>);
                    const trigger = getByRole("combobox", {
                        name: "Mock Label",
                    });
                    await user.click(trigger);
    ...
    describe("Popover portal", () => {
        ...
        it("will portal to the document body if the id does not match", async () => {
            ...
            <SelectWrapper selectedKey="batch-brew" isOpen portalContainerId={expectedContainerId}/>
          </>);
            };
            render(<SelectWithPortal />);
            await waitFor(() => {
