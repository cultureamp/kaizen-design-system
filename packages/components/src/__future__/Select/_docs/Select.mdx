...
You can also provide the `portalContainer` prop with an `Element` or `DocumentFragment` if you need the popover to render in a different location. This can circumvent issues that may arise with `z-index`.

<Canvas of={SelectStories.PortalContainer} />

It is important to note that a `relative` position should be applied to element that contains the portal, ie:
