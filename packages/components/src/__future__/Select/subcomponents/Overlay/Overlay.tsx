...
export const Overlay = <Option extends SelectOption>({ children, classNameOverride, ...restProps }: OverlayProps): JSX.Element => {
    const { state } = useSelectContext<Option>();
    // Handle events that should cause the menu to close,
    // e.g. blur, clicking outside, or pressing the escape key.
    const overlayRef = React.useRef<HTMLDivElement>(null);
    const { overlayProps } = useOverlay({ isDismissable: true, isOpen: state.isOpen, onClose: state.close }, overlayRef);
    ...
    return (<div ref={overlayRef} className={classNameOverride} {...overlayProps} {...restProps}>
