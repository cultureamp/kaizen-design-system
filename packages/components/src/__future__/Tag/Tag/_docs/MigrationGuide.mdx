...
As seen above, this replicable using the future `Tag`. Where the violation may occur is in [criteria 1.4.4](https://www.w3.org/WAI/WCAG21/Techniques/failures/F69), where by truncated text needs revealed / accessible via focus or some form of activation. Unless absolutely necessary, avoid truncation when migrating.

Note: the above example should be considered only as a starting point. We do no have guidance on this other than to follow the specifications outline in the WCAG guidelines.
