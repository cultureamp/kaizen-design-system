...
Originally controlled through `truncationWidth`, which set a `max-width` of inner text via inline styles, we have chosen not to adopt this in the future `Tag` as it presents a large risk of violating accessibility standards. While truncation can still be achieved in the `Tag` inner text via Tailwind or DOM manipulation, it is important to call out out this as a accessibility concern.
...
As seen above, this replicable using the future `Tag`. Where the violation may occur is in [criteria 1.4.4](https://www.w3.org/WAI/WCAG21/Techniques/failures/F69), where by truncated text needs revealed / accessible via focus or some form of activation. If possible, avoid truncation when migrating.
