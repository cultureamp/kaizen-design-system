...
While the `LiveIcon` is exported from `@kaizen/components` the animated implementation as a components, not its styles are avaiable available from kaio. As a temporary solution a `LiveIconComponent` can be created and added as a child of `Tag`.
