...
export const TextArea = ({ textAreaRef: propsTextAreaRef, status = "default", autogrow = false, reversed = false, rows = 3, defaultValue, value, disabled, onChange: propsOnChange, ...restProps }: TextAreaProps): JSX.Element => {
    const [textAreaHeight, setTextAreaHeight] = useState<string>("auto");
    const [parentHeight, setParentHeight] = useState<string>("auto");
    const [internalValue, setInternalValue] = useState<string | number | readonly string[] | undefined>(autogrow ? defaultValue : undefined);
    ...
    const getWrapperStyle = (): {
        minHeight: string;
    } | undefined => autogrow ? { minHeight: parentHeight } : undefined;
    const getTextAreaStyle = (): {
        height: string;
    } | undefined => autogrow ? { height: textAreaHeight } : undefined;
    const controlledValue = value || internalValue;
    return (<div className={styles.wrapper} style={getWrapperStyle()}>
      <textarea className={classnames(styles.textarea, styles[status], reversed ? styles.reversed : styles.default, disabled && styles.disabled)} rows={rows} onChange={onChange || propsOnChange} value={controlledValue} defaultValue={controlledValue ? undefined : defaultValue} 
    // ^ React throws a warning if you specify both a value and a defaultValue
    ref={textAreaRef} style={getTextAreaStyle()} disabled={disabled} {...restProps}/>
