...
export const transformSource = <TagName extends string | ImportModuleNameTagsMap>({
  sourceFile,
  astTransformer,
  tagName,
}: TransformConfig<TagName>): string => {
  const result = ts.transform(sourceFile, [(context) => astTransformer(context, tagName)])
