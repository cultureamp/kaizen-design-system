...
const run = (): void => {
  console.log(' ~(-_- ~) Running primitives → semantic tokens transformer (~ -_-)~')
  const targetDir = process.argv[2]
  if (!targetDir || targetDir.startsWith('--')) {
    console.error('Usage: kaizen-codemod <dir> migratePrimitivesToSemanticTokens')
    process.exit(1)
  }
  ...
  traverseDir(targetDir, (filePath) => {
    ...
    if (filePath.endsWith('.scss') || filePath.endsWith('.css')) {
      ...
      const { code, converted, skipped } = filePath.endsWith('.scss')
        ? transformScssSemanticColors(source)
        : transformCssSemanticColors(source)
    ...
    if (/\.(tsx|jsx|ts)$/.test(filePath)) {
      ...
      const { code, converted, skipped } = transformTailwindSemanticClasses(source, {
        prefix,
        scriptKind,
      })
      if (converted.length > 0) fs.writeFileSync(filePath, code, 'utf8')
  ...
  console.log('')
  console.log(`✅ Converted ${convertedCount} primitive color token(s) to semantic tokens.`)

  if (skips.length > 0) {
    console.log('')
    console.log(`⚠️  SKIPPED ${skips.length} color(s) with no confident semantic mapping.`)
    console.log('   Resolve these with the /semantic-colours-migration skill (stage 2):')
    skips.forEach((s) => console.log(`   - ${s.file}:${s.line} — ${s.detail}`))
  } else {
    console.log('No ambiguous colors detected — stage 2 may not be needed.')
  }
}
