...
*   Links:
    *   Avoid linking table cells. Instead, consider using a list of [Cards](/components/card). If you must include a link, place the link inside a cell, instead of linking the entire row.
*   **Expanded** **rows / cards**:
    *   Can be expanded or collapsed by an icon button containing a chevron icon.
    *   Expanded cards can contain either:
        *   A nested table: with aligned column headings and additional rows.
...
*   Arranging data:
    ...
    *   Filter: use a [Checkbox Field](/components/checkbox-field) in the table or [Select](/components/select) (default or secondary style) above the table.
