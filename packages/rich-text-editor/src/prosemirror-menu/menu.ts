...
export class MenuItem {
  ...
  render(view) {
    ...
    function update(state) {
      // console.log("selected", spec.select && spec.select(state))
      // console.log("enable", spec.enable && spec.enable(state))
      // console.log("active", spec.active && spec.active(state))
      if (spec.select) {
...
function combineUpdates(updates, nodes) {
  return state => {
    ...
    for (let i = 0; i < updates.length; i++) {
      ...
      // HERE: parent
      nodes[i].style.display = up ? "" : "inline-block"
...
export function renderGrouped(view, content) {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    for (let j = 0; j < items.length; j++) {
      ...
      // console.log(items[j].render(view))
