...
For a **row of selects** that you may use in a filter bar, you may use visually hidden labels for each individual select to be read by screen readers and other assistive devices, as well as a shared, visible label for the group (likely a `legend` for the `fieldset`), such as "Filters". When using visually hidden labels, use a good default for the pre-selected option to illustrate its use.
...
*   **Select control:** shows the currently selected option.
*   **Select dropdown container:** contains the available options. It is always shown above other elements, including the edges of modals.
...
There’s an option to turn on or off search in the input part. When the options are likely known and there are more than 10, use search.
