...
function migrateContent(content, filePath) {
  ...
  const sortedMappings = Object.entries(COLOR_MAPPINGS).sort(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isSass || isCss) {
      const cssVarRgbaPattern =
        /rgba\(var\(--color-([a-zA-Z]+-\d+)-rgb\),\s*([^)]+)\)/g;
    ...
    if (isSass) {
      for (const [legacy, newName] of sortedMappings) {
        ...
        const sassPattern = new RegExp(
          `\\$color-${legacy}(?!-rgb)\\b`,
          "g"
        );
        line = line.replace(sassPattern, `$color-${PLACEHOLDER_PREFIX}${newName}${PLACEHOLDER_SUFFIX}`);
      }
    }
    ...
    if (isSass || isCss) {
      for (const [legacy, newName] of sortedMappings) {
        ...
        line = line.replace(cssVarPattern, `--color-${PLACEHOLDER_PREFIX}${newName}${PLACEHOLDER_SUFFIX}`);
      }
    }
    ...
    if (isTsx) {
      for (const [legacy, newName] of sortedMappings) {
        ...
        line = line.replace(tailwindPattern, `$1$2-${PLACEHOLDER_PREFIX}${newName}${PLACEHOLDER_SUFFIX}$3`);
      }
    }
...
function findFiles(dir, extensions) {
  ...
  function walk(currentDir) {
    ...
    for (const entry of entries) {
      ...
      if (
        entry.isDirectory() &&
        !["node_modules", "dist", ".git", ".turbo", "coverage", "storybook-static", ".next", "build", "out"].includes(
          entry.name
        )
      ) {
        walk(fullPath);
