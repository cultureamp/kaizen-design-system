...
import { sanitizeName } from "@storybook/codemod/dist/lib/utils"
...
export default function transformer(file, api, options) {
  ...
  function convertToModuleExports(path, originalExports) {
    ...
    base
      ...
      .filter(
        add =>
          add.node.callee.property && add.node.callee.property.name === "add"
      )
      .filter(
        add =>
    ...
    adds.forEach(add => {
      ...
      if (add.node.arguments.length > 2) {
        ...
        if (storyParams) {
          storyAnnotations.push(
            j.property("init", j.identifier("parameters"), storyParams)
          )
        }
        if (storyDecorators) {
          storyAnnotations.push(
            j.property("init", j.identifier("decorators"), storyDecorators)
          )
        }
      }
  ...
  root
    .find(j.CallExpression)
    .filter(
      add => add.node.callee.property && add.node.callee.property.name === "add"
    )
    .filter(
      add =>
        add.node.arguments.length >= 2 &&
