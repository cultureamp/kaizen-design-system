---
title: Search Field
navTitle: Search Field (not built)
...
- Search/search field: "Search" describes the general behavior and, informally, the search field. "Search field" describes the main search component (label, input and button).
...
- **Search icon**: Use the search icon to help users identify search fields. Either display the search icon as a presentional icon inside the search input or as an Icon Button to submit.
...
- Let users submit search by clicking a button or hitting enter, unless it's a live-filtering search. For live-filtering search, update search results immediately and be mindful of performance.
...
- Combine search with Autocomplete where possible.

### Behavior

- **Global search**: try to use global search that searches all of the site instead of scoped search, according to newer studies on search usability.
...
- **Loading state**:
   ...
   - Consider if “debouncing” is necessary for rapidly changing search queries to maintain performance and responsive interaction.
- **Empty state**: Before a user has searched, consider giving them an indication of what to expect from a search. See Empty State component.
