...
* When there’s complex content or behaviour, use a [well](./well) instead. For example, showing deleted content with undo.
* When there’s content on the page but you need to inform the user, use an [inline notification](./inline-notification) instead.
* When there’s a change in state or context you need to inform the user about, use an [inline notification](./inline-notification) instead.
* When something negative has happened because the system failed or is a direct result of the user providing bad input, use an error [notification](./inline-notification) instead.
* Hero data pattern.
