...
- **Medium variant:**
    - Used when there are 4 or less pages.
    - Allow users to navigate to the previous and next set of items in a page list.
    - Does not include “Start” and “End” links.
- **Large variant:**
    - Used when there are 5 or more pages.
    - A non-interactive ellipsis is used to truncate pages. Truncation can exist between the first and last pages.
    - By default “Start” and “End” links are included as bookends to help the user navigate to the first and last page. These can be optionally removed.
...
- Use Pagination when there's a lot of screen real estate.
...
- Manual pagination may be less efficient than [Infinite Scroll](/guidelines/infinite-scroll) because it may require additional cognitive load to choose a page number or pagination button.
...
- On small and touch devices, consider using [Infinite Scroll](/guidelines/infinite-scroll) instead to avoid interrupting the flow with the need to tap a button as people scroll.
