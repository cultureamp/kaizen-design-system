...
Steps are selectable values (numerical or descriptive) along a slider range and can be visually indicated by vertical tick marks. 
- For a fine-grained range, hide tick marks to minimize visual clutter. For example, a range of 0%–100% with 1% steps.
- For a coarse-grained range, where the slider may snap to widely spaced intervals, you show a tick mark for every step so it is clear where the slider will snap to. 
