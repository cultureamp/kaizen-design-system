...
demoStoryId: components-form-slider--default-kaizen-site-demo
needToKnow:
...
<iframe style="border: none;" width="744" height="450" src="https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Ffile%2FeZKEE5kXbEMY3lx84oz8iN%2F%25E2%259D%25A4%25EF%25B8%258F-UI-Kit-Heart%3Fnode-id%3D16080%253A4049" allowfullscreen></iframe>

## Options

- **Default:**
    - In a default slider, the user can make a selection from a range of values, where there is no implication of progression from one side to the other.
- **Progress:**
    - In a progress slider, the user can make a selection from a range of values, aided by an exact indication of value. A sense of progress is implied, from one side to the other.
...
- Default sliders have no fill. The progress slider uses a fill to give the impression of building towards the maximum value. See [Progress track](https://github.com/components/slider/#progress-track).
...
A filled progress track is used in the progress slider. It follows the path of the regular track to indicate progress of the handle from the lowest/left value in the range to its current position.
...
- Use a progress slider when user input is required to provide the amount of progress. If progress updates automatically, consider using a [Progress Bar](/components/progress-bar) instead.
