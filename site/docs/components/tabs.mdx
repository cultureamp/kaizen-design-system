...
demoStoryId: components-tabs--uncontrolled
health:
  designed: true
  documented: true
  implemented: true
...
- **Separator**: To separate tabs from content, use whitespace or the edge of another component, such as Title Blocks or Containers, not a [Divider](/components/divider).
...
- Avoid tabs when the items perform an action. Instead, use a [Button](/components/button).
