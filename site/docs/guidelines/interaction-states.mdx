...
To indicate this to the user, the button triggering the action changes to a 'working' state while it waits. 

#### Behaviour

The working state of a button is pseudo-disabled. It visually appears disabled to indicate it isn't actionable, and it must retain focus for accessibility reasons. 

To avoid the button changing the position of nearby actionable controls, the working button must retain it's pre-working state width. The label becomes visually hidden when transitioning into this state, and a loading spinner is centred within the button.
