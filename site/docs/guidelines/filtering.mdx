...
*   **Positioning:** Top or left panel? Exposed filters (for example [Checkboxes](/components/checkbox-group) or [Date Picker](/components/date-picker)) exposed next to content, or button that reveals a Filter menu (for example panel of options that reveals itself, to help minimize content clutter).
...
*   Narrowing data by a pre-defined list of options e.g. Using a searchable select element for "City" where you can narrow to a city or department by starting to type characters to narrow and select from the the available options 
