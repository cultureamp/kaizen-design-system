...
*   **Positioning:** Top or left panel? Exposed filters (for exmaple [Checkboxes](/components/checkbox-group) or [Date Picker](/components/date-picker)) exposed next to content, or button that reveals a Filter menu (for example panel of options that reveals itself, to help minimize content clutter).
...
*   Narrowing data by by a pre-defined list of options e.g. Using a searchable select element for "City" where you can narrow to a city or department by starting to type characters to narrow and select from the the available options 


The select filter:

*   Could be live-updating, could have "Apply", "Cancel", "Reset" options
*   A select element may itself be 'searchable' and use autocomplete to narrow down from a large number of options, but the value filtered by the selected  predefined option (not the search string used to narrow those options) 
...
*   Narrowing data by pre-defined things – for example, Surveys have pre-defined status (Draft, Live, Closed)
...
*   There are too many results to show at once e.g. Surveys
...
[Search](/components/search-field) lets users quickly find relevant content. It allows a user to type a word or phrase to filter a large amount of data down to a known piece of content or to a narrower list of relevant content without navigation. Unlike a select filter, a search filter can match incomplete words and does not require a predefined list of options to select from.
...
*   Narrowing a set of results by part of a word or value e.g. finding people who work at the same company by filtering to domain part of the email address field 
...
In [data visualizations](/guidelines/data-visualization), such as a bar chart that show time on the X Axis:
