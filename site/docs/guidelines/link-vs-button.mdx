...
*   **Links that “open in a new tab” must use the “opens in new tab tooltip”**
*   **Cursors:**
    *   We choose to follow the Operating System (OS) defaults:
        *   Links use the pointer cursor that looks like a hand pointing (“cursor: pointer”)
        *   Buttons and other controls use the default cursor that looks like an arrow (“cursor: default”)
    *   The cursor indicates the semantic meaning and behavior of the element, not its visual appearance:
        *   Links styled as buttons (text on a solid background colour) still use the link cursor (“cursor: pointer”)
        *   Buttons styled to look like links (underlined text) use the button cursor (“cursor: default”)
        *   When a Secondary Action Button is used as a link, this can lead to a slightly confusing experience because the cursor changes over what looks like a button. Power users will recognise this as a signal that the button supports link interactions like opening in a new tab. This minor visual inconsistency can also prompt a conversation in the product team about whether the Secondary Action Button should be Link instead, which we believe is healthy.
    *   Until we have strong user research evidence that this approach is problematic, we will stick with this.
    *   A good article in support of this approach:
    ...
    *   A good article covering the contrary viewpoint, that all interactive elements should use “cursor: pointer”:
